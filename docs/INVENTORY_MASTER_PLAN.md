# Inventory Master Plan: May 2026 – January 2027

*Generated: May 3, 2026 · 8 SKUs · 7 channels · 9-month selling period + 30-day carry-over buffer*

---

## Executive Summary

We're entering the 9-month window from May through January — our most important selling period, with November and December accounting for the majority of annual volume. This plan models demand for all 7 channels (Amazon US FBA, Amazon CA FBA, US Shopify, CA Shopify, UK, EU, AU), sets a 30-day safety buffer for each, and determines exactly what stock needs to move and where — before peak season starts.

**The core rule:** US hubs (HBG/SLI/SAV/KCM) and the CA hub serve Shopify channels first. Only stock above Shopify needs can transfer to FBA. If total stock is still short after counting everything, we print — and new prints ship direct from the factory to the destination, never routed through a hub.

### Actions Required

#### 🖨️ Print Orders

**Kids Journal — 28,702 units**
All channels combined fall **13,873 units short** of covering May–January demand plus a 30-day buffer (75,813 available vs 89,686 needed). Peak month is Dec at 20,248 units globally. Printing:
  - **20,843 units** → Amazon US FBA *(direct from printer)*
  - **4,830 units** → Amazon CA FBA *(direct from printer)*
  - **3,029 units** → US Shopify *(direct from printer)*

**Know Me If You Can Cards — 6,730 units**
All channels combined fall **5,805 units short** of covering May–January demand plus a 30-day buffer (34,177 available vs 39,982 needed). Peak month is Nov at 14,995 units globally. Printing:
  - **2,642 units** → Amazon CA FBA *(direct from printer)*
  - **1,095 units** → CA Shopify *(direct from printer)*
  - **607 units** → UK *(direct from printer)*
  - **35 units** → EU *(direct from printer)*
  - **2,351 units** → AU *(direct from printer)*

#### 📦 Hub→FBA Transfers *(reposition surplus, no print needed)*

**Teen Journal** — globally covered (+25,717 units ahead), but FBA channels are light. Transferring hub surplus:
  - 1,038 units: US Hub → Amazon US FBA

**Sharing Joy Conversation Cards** — globally covered (+2,408 units ahead), but FBA channels are light. Transferring hub surplus:
  - 1,961 units: CA Hub → Amazon CA FBA

**Daily Journal (Teal)** — globally covered (+8,903 units ahead), but FBA channels are light. Transferring hub surplus:
  - 10,185 units: US Hub → Amazon US FBA
  - 1,429 units: CA Hub → Amazon CA FBA

**Daily Journal (Green)** — globally covered (+39,026 units ahead), but FBA channels are light. Transferring hub surplus:
  - 6,078 units: US Hub → Amazon US FBA
  - 954 units: CA Hub → Amazon CA FBA

**Adult Journal** — globally covered (+35,440 units ahead), but FBA channels are light. Transferring hub surplus:
  - 5,302 units: US Hub → Amazon US FBA
  - 190 units: CA Hub → Amazon CA FBA

**Dream Affirmation Cards** — globally covered (+11,747 units ahead), but FBA channels are light. Transferring hub surplus:
  - 3,514 units: US Hub → Amazon US FBA
  - 440 units: CA Hub → Amazon CA FBA

#### 🖨️ Top-Up Prints *(targeted fills where transfer routes are blocked)*

**Teen Journal:**
  - **7,032 units** → Amazon US FBA
  - **2,750 units** → Amazon CA FBA

**Sharing Joy Conversation Cards:**
  - **1,401 units** → Amazon US FBA
  - **901 units** → Amazon CA FBA
  - **2,514 units** → US Shopify
  - **430 units** → AU

**Daily Journal (Teal):**
  - **1,797 units** → AU

**Dream Affirmation Cards:**
  - **1,593 units** → Amazon CA FBA

#### ✈️ International Transfers *(UK surplus → AU/EU)*

**Kids Journal:**
  - 5,066 units: UK → AU

**Daily Journal (Teal):**
  - 1,221 units: UK → AU

**Dream Affirmation Cards:**
  - 523 units: UK → EU

### Summary Table

| SKU | Decision | Units | Notes |
| :--- | :--- | ---: | :--- |
| Kids Journal | 🖨️ Print run | 28,702 | Globally short 13,873 units · 5,066 UK→intl |
| Teen Journal | 📦 Transfer + 🖨️ top-up print | 10,820 | 1,038 transfer · 9,782 top-up print |
| Sharing Joy Conversation Cards | 📦 Transfer + 🖨️ top-up print | 7,207 | 1,961 transfer · 5,246 top-up print |
| Daily Journal (Teal) | 📦 Transfer + 🖨️ top-up print | 13,412 | 11,615 transfer · 1,797 top-up print · 1,221 UK→intl |
| Daily Journal (Green) | 📦 Hub→FBA transfer | 7,032 | Globally sufficient |
| Adult Journal | 📦 Hub→FBA transfer | 5,493 | Globally sufficient |
| Dream Affirmation Cards | 📦 Transfer + 🖨️ top-up print | 5,547 | 3,954 transfer · 1,593 top-up print · 523 UK→intl |
| Know Me If You Can Cards | 🖨️ Print run | 6,730 | Globally short 5,805 units |

---

## Section 1: How the Numbers Were Built

### Demand Forecast: Always Plan for the Worst Month We've Seen

For each channel and each month (May through January), we look at 2024 and 2025 actuals and take the **higher number**. If October 2024 was bigger than October 2025, we plan for October 2024. This means we're prepared for a strong Q4 — not just an average one.

*Example — Kids Journal Q4:*

| Month | 2024 | 2025 | Plan Uses |
| :--- | ---: | ---: | ---: |
| Oct | 3,247 | 2,770 | **3,247** ← 2024 |
| Nov | 8,953 | 17,726 | **17,726** ← 2025 |
| Dec | 19,957 | 20,248 | **20,248** ← 2025 |

### The 30-Day Safety Buffer

We don't plan to hit zero in January. The buffer is the average of what each channel sells in February, March, and April — one month's worth of stock that must still be sitting in the warehouse on February 1st, before the next replenishment arrives. This prevents stockouts in the gap between January and whenever the next order lands.

### The Print vs. Transfer Decision

For each SKU, we ask one question first: **does total stock across all channels cover total demand + buffers?**

- **Yes (globally sufficient):** No print run needed. We reposition hub surplus to FBA instead. Hubs reserve their Shopify demand + buffer; only the excess moves.
- **No (globally short):** A print run is ordered. New stock ships direct from the factory to each short channel — it never routes through a hub first.

This is a binary decision per SKU. We don't mix print runs with hub→FBA transfers for the same SKU. If we're printing, FBA gets stock from the printer. If we're not printing, FBA gets stock from hub surplus.

### Routing Rules

| Route | Allowed? | Why |
| :--- | :--- | :--- |
| UK → AU | ✅ Journals only | Cards can't use this route |
| UK → EU | ✅ All SKUs | |
| US Hub surplus → US FBA | ✅ If no print run | Shopify reserve must be maintained |
| CA Hub surplus → CA FBA | ✅ If no print run | Shopify reserve must be maintained |
| New print → hub → FBA | ❌ | Too slow; prints go direct |
| Canada supplier → US channels | ❌ | Restricted to CA channels only |

---

## Section 2: SKU-by-SKU Analysis

Each SKU section covers: the situation in plain language, the demand forecast, hub surplus math, the specific decision made, what would have happened without that decision, and the full rolling depletion forecast by channel.

---

### Kids Journal `EIDJ4100`

Peak demand hits in **Dec** at **20,248 units** globally. Even counting all warehouses — hubs, FBA, UK, AU — we're **13,873 units short** of covering every channel through January with a 30-day buffer (75,813 available vs 89,686 needed). Without a print run, **Amazon CA FBA** runs out of stock in **May 2026**, right before or during peak season. New stock is ordered and shipped direct from the factory to each short channel.

#### Monthly Demand Forecast

*Max of 2024 and 2025 actuals taken for each month. The higher year is used so we plan for the strongest Q4 we've seen.*

| Month | 2024 Actual | 2025 Actual | Plan Uses | Cumulative |
| :--- | ---: | ---: | ---: | ---: |
| May 2026 | 6,607 | 3,091 | **6,607** ← 2024 | 6,607 |
| Jun | 4,297 | 3,247 | **4,297** ← 2024 | 10,904 |
| Jul | 4,161 | 2,584 | **4,161** ← 2024 | 15,065 |
| Aug | 3,897 | 1,501 | **3,897** ← 2024 | 18,962 |
| Sep | 3,988 | 1,789 | **3,988** ← 2024 | 22,950 |
| Oct | 3,247 | 2,770 | **3,247** ← 2024 | 26,197 |
| Nov | 8,953 | 17,726 | **17,726** ← 2025 | 43,923 |
| Dec | 19,957 | 20,248 | **20,248** ← 2025 | 64,171 |
| Jan 2027 | 10,256 | 4,233 | **10,256** ← 2024 | 74,427 |
| **9-Month Total** | | | | **74,427** |

#### 30-Day Safety Buffer Calculation

*Average of Feb, Mar, Apr monthly sales — one month's worth of stock that must remain at end of January.*

| Month | 2024 | 2025 | Monthly Average |
| :--- | ---: | ---: | ---: |
| Feb | 8,761 | 3,682 | 5,074 |
| Mar | 8,221 | 3,184 | 5,107 |
| Apr | 5,733 | 4,147 | 4,940 |
| **30-day buffer (avg of 3 months)** | | | **5,040** |

> ⚠️ **Globally short: 13,873 units.** Total stock across all channels: 75,813 · Total need (demand + buffer): 89,686 · **Print run triggered.**

#### US & CA Channel Analysis

**Step 1 — How much hub stock is reserved for Shopify vs available for FBA?**

| | US Hubs (HBG / SLI / SAV / KCM) | CA Hub |
| :--- | ---: | ---: |
| Current stock at hub | 22,728 | 3,184 |
| Incoming supplier stock | +0 | +1,953 |
| **Total available** | **22,728** | **5,137** |
| Shopify demand May–Jan | 24,560 | 4,867 |
| 30-day Shopify buffer | 1,197 | 270 |
| **Shopify reserve (must keep)** | **25,757** | **5,137** |
| **Hub result** | **−3,029 shortfall — hub itself needs 3,029 more units** | **−0 shortfall — hub needs 0 more** |

**Step 2 — After hub transfer, is FBA covered?**

| | Amazon US FBA | Amazon CA FBA |
| :--- | ---: | ---: |
| Current FBA stock | 1,935 | 0 |
| Supplier stock | +0 | +306 |
| **Total FBA stock** | **1,935** | **306** |
| FBA demand May–Jan | 20,844 | 4,867 |
| 30-day FBA buffer | 1,934 | 270 |
| **FBA total need** | **22,778** | **5,137** |
| **FBA result** | **−20,843 still short** | **−4,831 still short** |

**US Hub monthly sell-through (US Shopify channel):**

| Month | Projected Sales | Hub Balance | vs Buffer |
| :--- | ---: | ---: | ---: |
| May | 2,174 | **23,583** | +22,386 |
| Jun | 1,214 | **22,369** | +21,172 |
| Jul | 811 | **21,558** | +20,361 |
| Aug | 1,054 | **20,504** | +19,307 |
| Sep | 1,022 | **19,482** | +18,285 |
| Oct | 897 | **18,585** | +17,388 |
| Nov | 6,699 | **11,886** | +10,689 |
| Dec | 7,994 | **3,892** | +2,695 |
| Jan | 2,695 | **1,197** | +0 |

> Jan 2027 hub balance: **1,197** · Buffer target: 1,197 · ✅ Hub ends above buffer

#### Per-Channel Stock Check

| Channel | 9-Mo Demand | 30-Day Buffer | Total Need | Stock on Hand | Decision |
| :--- | ---: | ---: | ---: | ---: | :--- |
| Amazon US FBA | 20,844 | 1,934 | 22,778 | 1,935 | 🖨️ Print 20,843 direct |
| Amazon CA FBA | 4,867 | 270 | 5,137 | 0 | 📦 306 supplier, 🖨️ Print 4,830 direct |
| US Shopify | 24,560 | 1,197 | 25,757 | 22,728 (HBG+SLI+SAV+KCM) | 🖨️ Print 3,029 direct |
| CA Shopify | 4,867 | 270 | 5,137 | 3,184 (CA hub) | ✅ Covered by current stock + 1,953 supplier |
| UK | 12,661 | 796 | 13,457 | 33,137 | ✅ Covered by current stock |
| EU | 3,246 | 127 | 3,373 | 3,590 | ✅ Covered by current stock |
| AU | 13,282 | 763 | 14,045 | 8,979 | ✅ Covered by current stock |

> **UK:** 33,137 current − 5,066 transferred = **28,071 remaining** vs UK demand 12,661 · ✅ covered

> **Decision: Print 28,702 units** (20,843 to Amazon US FBA + 4,830 to Amazon CA FBA + 3,029 to US Shopify) · ship direct from factory

**Without this print run — what would break:**

| Channel | Stock Without Print | Print Adds | Consequence |
| :--- | ---: | ---: | :--- |
| Amazon US FBA | 1,935 | 20,843 | 🚨 Runs out of stock in **Jun** |
| Amazon CA FBA | 307 | 4,830 | 🚨 Runs out of stock in **May 2026** |
| US Shopify | 22,728 | 3,029 | 🚨 Runs out of stock in **Jan 2027** |

#### Rolling Depletion Forecast

*Starting stock = current inventory after all transfers and print runs arrive. Each month we subtract max projected sales. The goal: end January above the 30-day buffer line.*

#### Amazon US FBA

*Starting stock: **22,778** (1,935 current + + 20,843 print)*

*30-day buffer target: **1,934 units** (Feb avg 1,747 + Mar avg 1,897 + Apr avg 2,158 = 1,934)*

| Month | Projected Sales | Ending Stock | vs Buffer | Weeks of Cover |
| :--- | ---: | ---: | ---: | ---: |
| May 2026 | 1,832 | **20,946** | +19,012 | 69.0w |
| Jun | 1,344 | **19,602** | +17,668 | 53.1w |
| Jul | 1,582 | **18,020** | +16,086 | 54.8w |
| Aug | 1,456 | **16,564** | +14,630 | 48.3w |
| Sep | 1,520 | **15,044** | +13,110 | 47.9w |
| Oct | 1,345 | **13,699** | +11,765 | 27.9w |
| Nov | 2,176 | **11,523** | +9,589 | 7.2w |
| Dec | 6,862 | **4,661** | +2,727 | 7.6w |
| Jan 2027 | 2,727 | **1,934** | +0 | 13.3w |

> **Jan 2027 ending: 1,934 units** · Buffer target: 1,934 · ✅

#### Amazon CA FBA

*Starting stock: **5,137** (0 current + + 5,137 print)*

*30-day buffer target: **270 units** (Feb avg 292 + Mar avg 300 + Apr avg 218 = 270)*

| Month | Projected Sales | Ending Stock | vs Buffer | Weeks of Cover |
| :--- | ---: | ---: | ---: | ---: |
| May 2026 | 666 | **4,471** | +4,201 | 47.6w |
| Jun | 416 | **4,055** | +3,785 | 77.6w |
| Jul | 224 | **3,831** | +3,561 | 50.1w |
| Aug | 339 | **3,492** | +3,222 | 65.3w |
| Sep | 237 | **3,255** | +2,985 | 78.8w |
| Oct | 177 | **3,078** | +2,808 | 12.5w |
| Nov | 1,093 | **1,985** | +1,715 | 6.7w |
| Dec | 1,278 | **707** | +437 | 7.2w |
| Jan 2027 | 437 | **270** | +0 | 13.3w |

> **Jan 2027 ending: 270 units** · Buffer target: 270 · ✅

#### US Shopify

*Starting stock: **25,757** (HBG+SLI+SAV+KCM: 22,728 + 3,029 print)*

*30-day buffer target: **1,197 units** (Feb avg 1,170 + Mar avg 1,192 + Apr avg 1,231 = 1,197)*

| Month | Projected Sales | Ending Stock | vs Buffer | Weeks of Cover |
| :--- | ---: | ---: | ---: | ---: |
| May 2026 | 2,174 | **23,583** | +22,386 | 86.0w |
| Jun | 1,214 | **22,369** | +21,172 | 118.2w |
| Jul | 811 | **21,558** | +20,361 | 90.6w |
| Aug | 1,054 | **20,504** | +19,307 | 88.9w |
| Sep | 1,022 | **19,482** | +18,285 | 93.1w |
| Oct | 897 | **18,585** | +17,388 | 12.3w |
| Nov | 6,699 | **11,886** | +10,689 | 6.4w |
| Dec | 7,994 | **3,892** | +2,695 | 6.4w |
| Jan 2027 | 2,695 | **1,197** | +0 | 13.3w |

> **Jan 2027 ending: 1,197 units** · Buffer target: 1,197 · ✅

#### CA Shopify

*Starting stock: **5,137** (CA hub: 3,184 + 1,953 supplier)*

*30-day buffer target: **270 units** (Feb avg 292 + Mar avg 300 + Apr avg 218 = 270)*

| Month | Projected Sales | Ending Stock | vs Buffer | Weeks of Cover |
| :--- | ---: | ---: | ---: | ---: |
| May 2026 | 666 | **4,471** | +4,201 | 47.6w |
| Jun | 416 | **4,055** | +3,785 | 77.6w |
| Jul | 224 | **3,831** | +3,561 | 50.1w |
| Aug | 339 | **3,492** | +3,222 | 65.3w |
| Sep | 237 | **3,255** | +2,985 | 78.8w |
| Oct | 177 | **3,078** | +2,808 | 12.5w |
| Nov | 1,093 | **1,985** | +1,715 | 6.7w |
| Dec | 1,278 | **707** | +437 | 7.2w |
| Jan 2027 | 437 | **270** | +0 | 13.3w |

> **Jan 2027 ending: 270 units** · Buffer target: 270 · ✅

#### UK

*Starting stock: **28,071** (33,137 current − 5,066 transferred out)*

*30-day buffer target: **796 units** (Feb avg 1,148 + Mar avg 760 + Apr avg 480 = 796)*

| Month | Projected Sales | Ending Stock | vs Buffer | Weeks of Cover |
| :--- | ---: | ---: | ---: | ---: |
| May 2026 | 846 | **27,225** | +26,428 | 227.1w |
| Jun | 531 | **26,694** | +25,897 | 143.2w |
| Jul | 799 | **25,895** | +25,098 | 298.6w |
| Aug | 384 | **25,511** | +24,714 | 298.9w |
| Sep | 378 | **25,133** | +24,336 | 325.4w |
| Oct | 331 | **24,802** | +24,005 | 35.1w |
| Nov | 3,127 | **21,675** | +20,878 | 45.8w |
| Dec | 2,027 | **19,648** | +18,851 | 20.5w |
| Jan 2027 | 4,238 | **15,410** | +14,613 | 257.0w |

> **Jan 2027 ending: 15,410 units** · Buffer target: 796 · ✅

#### AU

*Starting stock: **14,045** (8,979 current + + 5,066 transfer)*

*30-day buffer target: **763 units** (Feb avg 651 + Mar avg 841 + Apr avg 796 = 763)*

| Month | Projected Sales | Ending Stock | vs Buffer | Weeks of Cover |
| :--- | ---: | ---: | ---: | ---: |
| May 2026 | 1,089 | **12,956** | +12,193 | 72.4w |
| Jun | 792 | **12,164** | +11,401 | 68.4w |
| Jul | 762 | **11,402** | +10,639 | 76.0w |
| Aug | 664 | **10,738** | +9,975 | 57.2w |
| Sep | 831 | **9,907** | +9,144 | 62.5w |
| Oct | 679 | **9,228** | +8,465 | 9.8w |
| Nov | 4,180 | **5,048** | +4,285 | 6.1w |
| Dec | 3,569 | **1,479** | +716 | 9.1w |
| Jan 2027 | 716 | **763** | +0 | 13.3w |

> **Jan 2027 ending: 763 units** · Buffer target: 763 · ✅

---

### Teen Journal `EIDJ2100`

Peak demand hits in **Dec** at **14,818 units** globally. We have **84,718 units** across all channels against a total need of **59,001** — a surplus of **25,717 units**. No new print is required. However, the stock isn't in the right place: most of it sits in the UK or US hubs while FBA channels are light. The plan repositions hub surplus into FBA and uses small targeted top-up prints where transfer routes are blocked.

#### Monthly Demand Forecast

*Max of 2024 and 2025 actuals taken for each month. The higher year is used so we plan for the strongest Q4 we've seen.*

| Month | 2024 Actual | 2025 Actual | Plan Uses | Cumulative |
| :--- | ---: | ---: | ---: | ---: |
| May 2026 | 4,405 | 3,289 | **4,405** ← 2024 | 4,405 |
| Jun | 2,871 | 4,591 | **4,591** ← 2025 | 8,996 |
| Jul | 2,683 | 3,629 | **3,629** ← 2025 | 12,625 |
| Aug | 2,613 | 2,390 | **2,613** ← 2024 | 15,238 |
| Sep | 2,977 | 2,177 | **2,977** ← 2024 | 18,215 |
| Oct | 2,210 | 1,832 | **2,210** ← 2024 | 20,425 |
| Nov | 6,608 | 7,079 | **7,079** ← 2025 | 27,504 |
| Dec | 14,818 | 7,914 | **14,818** ← 2024 | 42,322 |
| Jan 2027 | 5,538 | 5,167 | **5,538** ← 2024 | 47,860 |
| **9-Month Total** | | | | **47,860** |

#### 30-Day Safety Buffer Calculation

*Average of Feb, Mar, Apr monthly sales — one month's worth of stock that must remain at end of January.*

| Month | 2024 | 2025 | Monthly Average |
| :--- | ---: | ---: | ---: |
| Feb | 5,340 | 5,232 | 4,396 |
| Mar | 4,029 | 3,920 | 3,756 |
| Apr | 2,609 | 2,559 | 2,584 |
| **30-day buffer (avg of 3 months)** | | | **3,579** |

> ✅ **Globally sufficient: +25,717 units ahead.** Total stock: 84,718 · Total need: 59,001 · **No print run needed.**

#### US & CA Channel Analysis

**Step 1 — How much hub stock is reserved for Shopify vs available for FBA?**

| | US Hubs (HBG / SLI / SAV / KCM) | CA Hub |
| :--- | ---: | ---: |
| Current stock at hub | 14,693 | 1,723 |
| Incoming supplier stock | +0 | +2,289 |
| **Total available** | **14,693** | **4,012** |
| Shopify demand May–Jan | 13,031 | 3,837 |
| 30-day Shopify buffer | 623 | 175 |
| **Shopify reserve (must keep)** | **13,654** | **4,012** |
| **Hub result** | **+1,039 surplus → transfer 1,038 to US FBA** | **−0 shortfall — hub needs 0 more** |

**Step 2 — After hub transfer, is FBA covered?**

| | Amazon US FBA | Amazon CA FBA |
| :--- | ---: | ---: |
| Current FBA stock | 10,284 | 0 |
| Transfer in from hub | +1,038 | +0 |
| Supplier stock | +0 | +1,261 |
| **Total FBA stock** | **11,322** | **1,261** |
| FBA demand May–Jan | 17,044 | 3,837 |
| 30-day FBA buffer | 1,310 | 175 |
| **FBA total need** | **18,354** | **4,012** |
| **FBA result** | **−7,032 still short** | **−2,751 still short** |

**US Hub monthly sell-through (US Shopify channel):**

| Month | Projected Sales | Hub Balance | vs Buffer |
| :--- | ---: | ---: | ---: |
| May | 1,054 | **12,601** | +11,978 |
| Jun | 999 | **11,602** | +10,979 |
| Jul | 1,025 | **10,577** | +9,954 |
| Aug | 648 | **9,929** | +9,306 |
| Sep | 704 | **9,225** | +8,602 |
| Oct | 853 | **8,372** | +7,749 |
| Nov | 1,768 | **6,604** | +5,981 |
| Dec | 4,742 | **1,862** | +1,239 |
| Jan | 1,238 | **624** | +1 |

> Jan 2027 hub balance: **624** · Buffer target: 623 · ✅ Hub ends above buffer

#### Per-Channel Stock Check

| Channel | 9-Mo Demand | 30-Day Buffer | Total Need | Stock on Hand | Decision |
| :--- | ---: | ---: | ---: | ---: | :--- |
| Amazon US FBA | 17,044 | 1,310 | 18,354 | 10,284 | ✅ Covered by current stock |
| Amazon CA FBA | 3,837 | 175 | 4,012 | 0 | ✅ Covered by current stock + 1,261 supplier |
| US Shopify | 13,031 | 623 | 13,654 | 14,693 (HBG+SLI+SAV+KCM) | Reserve 13,654 for Shopify · Transfer 1,038 surplus → US FBA |
| CA Shopify | 3,837 | 175 | 4,012 | 1,723 (CA hub) | ✅ Covered by current stock + 2,289 supplier |
| UK | 7,625 | 864 | 8,489 | 41,324 | ✅ Covered by current stock |
| EU | 1,292 | 139 | 1,431 | 2,162 | ✅ Covered by current stock |
| AU | 8,532 | 515 | 9,047 | 10,981 | ✅ Covered by current stock |
> **Decision: No full print run.** Hub surplus transfers proceed · Top-up prints: 7,032 to Amazon US FBA + 2,750 to Amazon CA FBA

#### Rolling Depletion Forecast

*Starting stock = current inventory after all transfers and print runs arrive. Each month we subtract max projected sales. The goal: end January above the 30-day buffer line.*

#### Amazon US FBA

*Starting stock: **18,354** (10,284 current + + 8,070 top-up print)*

*30-day buffer target: **1,310 units** (Feb avg 1,237 + Mar avg 1,444 + Apr avg 1,249 = 1,310)*

| Month | Projected Sales | Ending Stock | vs Buffer | Weeks of Cover |
| :--- | ---: | ---: | ---: | ---: |
| May 2026 | 1,880 | **16,474** | +15,164 | 43.7w |
| Jun | 1,671 | **14,803** | +13,493 | 48.7w |
| Jul | 1,302 | **13,501** | +12,191 | 46.3w |
| Aug | 1,291 | **12,210** | +10,900 | 37.1w |
| Sep | 1,459 | **10,751** | +9,441 | 51.9w |
| Oct | 888 | **9,863** | +8,553 | 22.0w |
| Nov | 1,982 | **7,881** | +6,571 | 7.0w |
| Dec | 4,858 | **3,023** | +1,713 | 7.8w |
| Jan 2027 | 1,713 | **1,310** | +0 | 13.3w |

> **Jan 2027 ending: 1,310 units** · Buffer target: 1,310 · ✅

#### Amazon CA FBA

*Starting stock: **4,012** (0 current + + 4,012 top-up print)*

*30-day buffer target: **175 units** (Feb avg 258 + Mar avg 191 + Apr avg 75 = 175)*

| Month | Projected Sales | Ending Stock | vs Buffer | Weeks of Cover |
| :--- | ---: | ---: | ---: | ---: |
| May 2026 | 221 | **3,791** | +3,616 | 29.2w |
| Jun | 575 | **3,216** | +3,041 | 54.5w |
| Jul | 253 | **2,963** | +2,788 | 72.5w |
| Aug | 181 | **2,782** | +2,607 | 49.3w |
| Sep | 250 | **2,532** | +2,357 | 65.0w |
| Oct | 167 | **2,365** | +2,190 | 14.2w |
| Nov | 737 | **1,628** | +1,453 | 9.3w |
| Dec | 752 | **876** | +701 | 5.5w |
| Jan 2027 | 701 | **175** | +0 | 13.3w |

> **Jan 2027 ending: 175 units** · Buffer target: 175 · ✅

#### US Shopify

*Starting stock: **13,654** (HBG+SLI+SAV+KCM: 14,693 − 1,038 transferred to US FBA)*

*30-day buffer target: **623 units** (Feb avg 732 + Mar avg 576 + Apr avg 563 = 623)*

| Month | Projected Sales | Ending Stock | vs Buffer | Weeks of Cover |
| :--- | ---: | ---: | ---: | ---: |
| May 2026 | 1,054 | **12,600** | +11,977 | 55.9w |
| Jun | 999 | **11,601** | +10,978 | 48.5w |
| Jul | 1,025 | **10,576** | +9,953 | 72.3w |
| Aug | 648 | **9,928** | +9,305 | 62.5w |
| Sep | 704 | **9,224** | +8,601 | 46.3w |
| Oct | 853 | **8,371** | +7,748 | 21.0w |
| Nov | 1,768 | **6,603** | +5,980 | 6.0w |
| Dec | 4,742 | **1,861** | +1,238 | 6.7w |
| Jan 2027 | 1,238 | **623** | +0 | 13.3w |

> **Jan 2027 ending: 623 units** · Buffer target: 623 · ✅

#### CA Shopify

*Starting stock: **4,012** (CA hub: 1,723 + 2,289 supplier)*

*30-day buffer target: **175 units** (Feb avg 258 + Mar avg 191 + Apr avg 75 = 175)*

| Month | Projected Sales | Ending Stock | vs Buffer | Weeks of Cover |
| :--- | ---: | ---: | ---: | ---: |
| May 2026 | 221 | **3,791** | +3,616 | 29.2w |
| Jun | 575 | **3,216** | +3,041 | 54.5w |
| Jul | 253 | **2,963** | +2,788 | 72.5w |
| Aug | 181 | **2,782** | +2,607 | 49.3w |
| Sep | 250 | **2,532** | +2,357 | 65.0w |
| Oct | 167 | **2,365** | +2,190 | 14.2w |
| Nov | 737 | **1,628** | +1,453 | 9.3w |
| Dec | 752 | **876** | +701 | 5.5w |
| Jan 2027 | 701 | **175** | +0 | 13.3w |

> **Jan 2027 ending: 175 units** · Buffer target: 175 · ✅

#### UK

*Starting stock: **41,324** (41,324 current)*

*30-day buffer target: **864 units** (Feb avg 1,437 + Mar avg 753 + Apr avg 402 = 864)*

| Month | Projected Sales | Ending Stock | vs Buffer | Weeks of Cover |
| :--- | ---: | ---: | ---: | ---: |
| May 2026 | 697 | **40,627** | +39,763 | 342.1w |
| Jun | 526 | **40,101** | +39,237 | 467.0w |
| Jul | 368 | **39,733** | +38,869 | 775.2w |
| Aug | 227 | **39,506** | +38,642 | 622.6w |
| Sep | 281 | **39,225** | +38,361 | 485.9w |
| Oct | 346 | **38,879** | +38,015 | 136.6w |
| Nov | 1,260 | **37,619** | +36,755 | 67.7w |
| Dec | 2,381 | **35,238** | +34,374 | 101.4w |
| Jan 2027 | 1,539 | **33,699** | +32,835 | 518.2w |

> **Jan 2027 ending: 33,699 units** · Buffer target: 864 · ✅

#### AU

*Starting stock: **10,981** (10,981 current)*

*30-day buffer target: **515 units** (Feb avg 619 + Mar avg 655 + Apr avg 271 = 515)*

| Month | Projected Sales | Ending Stock | vs Buffer | Weeks of Cover |
| :--- | ---: | ---: | ---: | ---: |
| May 2026 | 673 | **10,308** | +9,793 | 51.8w |
| Jun | 882 | **9,426** | +8,911 | 62.7w |
| Jul | 644 | **8,782** | +8,267 | 66.0w |
| Aug | 589 | **8,193** | +7,678 | 63.0w |
| Sep | 576 | **7,617** | +7,102 | 47.8w |
| Oct | 683 | **6,934** | +6,419 | 18.3w |
| Nov | 1,680 | **5,254** | +4,739 | 10.8w |
| Dec | 2,085 | **3,169** | +2,654 | 19.5w |
| Jan 2027 | 720 | **2,449** | +1,934 | 63.1w |

> **Jan 2027 ending: 2,449 units** · Buffer target: 515 · ✅

---

### Sharing Joy Conversation Cards `EIDC2000`

Peak demand hits in **Dec** at **11,695 units** globally. We have **59,022 units** across all channels against a total need of **56,614** — a surplus of **2,408 units**. No new print is required. However, the stock isn't in the right place: most of it sits in the UK or US hubs while FBA channels are light. The plan repositions hub surplus into FBA and uses small targeted top-up prints where transfer routes are blocked.

#### Monthly Demand Forecast

*Max of 2024 and 2025 actuals taken for each month. The higher year is used so we plan for the strongest Q4 we've seen.*

| Month | 2024 Actual | 2025 Actual | Plan Uses | Cumulative |
| :--- | ---: | ---: | ---: | ---: |
| May 2026 | — | 3,196 | **3,196** ← 2025 | 3,196 |
| Jun | — | 3,042 | **3,042** ← 2025 | 6,238 |
| Jul | — | 2,720 | **2,720** ← 2025 | 8,958 |
| Aug | — | 1,938 | **1,938** ← 2025 | 10,896 |
| Sep | 2,503 | 1,509 | **2,503** ← 2024 | 13,399 |
| Oct | 2,350 | 2,668 | **2,668** ← 2025 | 16,067 |
| Nov | 4,363 | 9,134 | **9,134** ← 2025 | 25,201 |
| Dec | 10,462 | 11,695 | **11,695** ← 2025 | 36,896 |
| Jan 2027 | — | 10,908 | **10,908** ← 2025 | 47,804 |
| **9-Month Total** | | | | **47,804** |

#### 30-Day Safety Buffer Calculation

*Average of Feb, Mar, Apr monthly sales — one month's worth of stock that must remain at end of January.*

| Month | 2024 | 2025 | Monthly Average |
| :--- | ---: | ---: | ---: |
| Feb | — | 2,937 | 2,326 |
| Mar | — | 3,975 | 2,758 |
| Apr | — | 5,351 | 5,351 |
| **30-day buffer (avg of 3 months)** | | | **3,478** |

> ✅ **Globally sufficient: +2,408 units ahead.** Total stock: 59,022 · Total need: 56,614 · **No print run needed.**

#### US & CA Channel Analysis

**Step 1 — How much hub stock is reserved for Shopify vs available for FBA?**

| | US Hubs (HBG / SLI / SAV / KCM) | CA Hub |
| :--- | ---: | ---: |
| Current stock at hub | 18,064 | 4,825 |
| Shopify demand May–Jan | 19,159 | 2,698 |
| 30-day Shopify buffer | 1,419 | 165 |
| **Shopify reserve (must keep)** | **20,578** | **2,863** |
| **Hub result** | **−2,514 shortfall — hub itself needs 2,514 more units** | **+1,962 surplus → transfer 1,961 to CA FBA** |

**Step 2 — After hub transfer, is FBA covered?**

| | Amazon US FBA | Amazon CA FBA |
| :--- | ---: | ---: |
| Current FBA stock | 9,662 | 0 |
| Transfer in from hub | +0 | +1,961 |
| Supplier stock | +5,500 | +0 |
| **Total FBA stock** | **15,162** | **1,961** |
| FBA demand May–Jan | 15,313 | 2,698 |
| 30-day FBA buffer | 1,250 | 165 |
| **FBA total need** | **16,563** | **2,863** |
| **FBA result** | **−1,401 still short** | **−902 still short** |

**US Hub monthly sell-through (US Shopify channel):**

| Month | Projected Sales | Hub Balance | vs Buffer |
| :--- | ---: | ---: | ---: |
| May | 1,141 | **19,437** | +18,018 |
| Jun | 1,161 | **18,276** | +16,857 |
| Jul | 939 | **17,337** | +15,918 |
| Aug | 652 | **16,685** | +15,266 |
| Sep | 1,465 | **15,220** | +13,801 |
| Oct | 1,532 | **13,688** | +12,269 |
| Nov | 2,293 | **11,395** | +9,976 |
| Dec | 4,642 | **6,753** | +5,334 |
| Jan | 5,334 | **1,419** | +0 |

> Jan 2027 hub balance: **1,419** · Buffer target: 1,419 · ✅ Hub ends above buffer

#### Per-Channel Stock Check

| Channel | 9-Mo Demand | 30-Day Buffer | Total Need | Stock on Hand | Decision |
| :--- | ---: | ---: | ---: | ---: | :--- |
| Amazon US FBA | 15,313 | 1,250 | 16,563 | 9,662 | ✅ Covered by current stock + 5,500 supplier |
| Amazon CA FBA | 2,698 | 165 | 2,863 | 0 | ✅ Covered by current stock |
| US Shopify | 19,159 | 1,419 | 20,578 | 18,064 (HBG+SLI+SAV+KCM) | ✅ Covered by current stock |
| CA Shopify | 2,698 | 165 | 2,863 | 4,825 (CA hub) | Reserve 2,863 for Shopify · Transfer 1,961 surplus → CA FBA |
| UK | 5,357 | 203 | 5,560 | 10,071 | ✅ Covered by current stock |
| EU | 1,336 | 43 | 1,379 | 4,523 | ✅ Covered by current stock |
| AU | 6,410 | 397 | 6,807 | 6,377 | ✅ Covered by current stock |
> **Decision: No full print run.** Hub surplus transfers proceed · Top-up prints: 1,401 to Amazon US FBA + 901 to Amazon CA FBA + 2,514 to US Shopify + 430 to AU

#### Rolling Depletion Forecast

*Starting stock = current inventory after all transfers and print runs arrive. Each month we subtract max projected sales. The goal: end January above the 30-day buffer line.*

#### Amazon US FBA

*Starting stock: **16,563** (9,662 current + + 6,901 top-up print)*

*30-day buffer target: **1,250 units** (Feb avg 860 + Mar avg 1,155 + Apr avg 1,735 = 1,250)*

| Month | Projected Sales | Ending Stock | vs Buffer | Weeks of Cover |
| :--- | ---: | ---: | ---: | ---: |
| May 2026 | 1,375 | **15,188** | +13,938 | 58.9w |
| Jun | 1,142 | **14,046** | +12,796 | 56.5w |
| Jul | 1,065 | **12,981** | +11,731 | 67.4w |
| Aug | 853 | **12,128** | +10,878 | 86.8w |
| Sep | 619 | **11,509** | +10,259 | 52.1w |
| Oct | 946 | **10,563** | +9,313 | 22.9w |
| Nov | 2,042 | **8,521** | +7,271 | 9.5w |
| Dec | 3,836 | **4,685** | +3,435 | 6.0w |
| Jan 2027 | 3,435 | **1,250** | +0 | 13.3w |

> **Jan 2027 ending: 1,250 units** · Buffer target: 1,250 · ✅

#### Amazon CA FBA

*Starting stock: **2,863** (0 current + + 2,863 top-up print)*

*30-day buffer target: **165 units** (Feb avg 127 + Mar avg 92 + Apr avg 276 = 165)*

| Month | Projected Sales | Ending Stock | vs Buffer | Weeks of Cover |
| :--- | ---: | ---: | ---: | ---: |
| May 2026 | 222 | **2,641** | +2,476 | 54.7w |
| Jun | 214 | **2,427** | +2,262 | 51.8w |
| Jul | 201 | **2,226** | +2,061 | 131.4w |
| Aug | 75 | **2,151** | +1,986 | 33.9w |
| Sep | 281 | **1,870** | +1,705 | 39.1w |
| Oct | 205 | **1,665** | +1,500 | 14.7w |
| Nov | 503 | **1,162** | +997 | 8.7w |
| Dec | 570 | **592** | +427 | 6.1w |
| Jan 2027 | 427 | **165** | +0 | 13.3w |

> **Jan 2027 ending: 165 units** · Buffer target: 165 · ✅

#### US Shopify

*Starting stock: **20,578** (HBG+SLI+SAV+KCM: 18,064 + 2,514 top-up print)*

*30-day buffer target: **1,419 units** (Feb avg 726 + Mar avg 922 + Apr avg 2,608 = 1,419)*

| Month | Projected Sales | Ending Stock | vs Buffer | Weeks of Cover |
| :--- | ---: | ---: | ---: | ---: |
| May 2026 | 1,141 | **19,437** | +18,018 | 74.1w |
| Jun | 1,161 | **18,276** | +16,857 | 83.4w |
| Jul | 939 | **17,337** | +15,918 | 117.8w |
| Aug | 652 | **16,685** | +15,266 | 50.4w |
| Sep | 1,465 | **15,220** | +13,801 | 42.6w |
| Oct | 1,532 | **13,688** | +12,269 | 26.4w |
| Nov | 2,293 | **11,395** | +9,976 | 10.5w |
| Dec | 4,642 | **6,753** | +5,334 | 5.6w |
| Jan 2027 | 5,334 | **1,419** | +0 | 13.3w |

> **Jan 2027 ending: 1,419 units** · Buffer target: 1,419 · ✅

#### CA Shopify

*Starting stock: **2,863** (CA hub: 4,825 − 1,961 to CA FBA)*

*30-day buffer target: **165 units** (Feb avg 127 + Mar avg 92 + Apr avg 276 = 165)*

| Month | Projected Sales | Ending Stock | vs Buffer | Weeks of Cover |
| :--- | ---: | ---: | ---: | ---: |
| May 2026 | 222 | **2,641** | +2,476 | 54.7w |
| Jun | 214 | **2,427** | +2,262 | 51.8w |
| Jul | 201 | **2,226** | +2,061 | 131.4w |
| Aug | 75 | **2,151** | +1,986 | 33.9w |
| Sep | 281 | **1,870** | +1,705 | 39.1w |
| Oct | 205 | **1,665** | +1,500 | 14.7w |
| Nov | 503 | **1,162** | +997 | 8.7w |
| Dec | 570 | **592** | +427 | 6.1w |
| Jan 2027 | 427 | **165** | +0 | 13.3w |

> **Jan 2027 ending: 165 units** · Buffer target: 165 · ✅

#### UK

*Starting stock: **10,071** (10,071 current)*

*30-day buffer target: **203 units** (Feb avg 226 + Mar avg 171 + Apr avg 214 = 203)*

| Month | Projected Sales | Ending Stock | vs Buffer | Weeks of Cover |
| :--- | ---: | ---: | ---: | ---: |
| May 2026 | 8 | **10,063** | +9,859 | 2345.5w |
| Jun | 19 | **10,044** | +9,840 | 3311.2w |
| Jul | 13 | **10,031** | +9,827 | 5552.9w |
| Aug | 8 | **10,023** | +9,819 | 160.8w |
| Sep | 276 | **9,747** | +9,543 | 204.8w |
| Oct | 204 | **9,543** | +9,339 | 18.0w |
| Nov | 2,346 | **7,197** | +6,993 | 17.6w |
| Dec | 1,757 | **5,440** | +5,236 | 33.2w |
| Jan 2027 | 726 | **4,714** | +4,510 | 307.3w |

> **Jan 2027 ending: 4,714 units** · Buffer target: 203 · ✅

#### AU

*Starting stock: **6,807** (6,377 current + + 430 top-up print)*

*30-day buffer target: **397 units** (Feb avg 337 + Mar avg 369 + Apr avg 486 = 397)*

| Month | Projected Sales | Ending Stock | vs Buffer | Weeks of Cover |
| :--- | ---: | ---: | ---: | ---: |
| May 2026 | 441 | **6,366** | +5,968 | 56.8w |
| Jun | 496 | **5,870** | +5,472 | 52.5w |
| Jul | 479 | **5,391** | +4,994 | 69.6w |
| Aug | 343 | **5,048** | +4,650 | 46.5w |
| Sep | 481 | **4,567** | +4,170 | 28.7w |
| Oct | 683 | **3,884** | +3,486 | 12.5w |
| Nov | 1,381 | **2,503** | +2,106 | 9.0w |
| Dec | 1,194 | **1,309** | +912 | 6.4w |
| Jan 2027 | 912 | **397** | +0 | 13.3w |

> **Jan 2027 ending: 397 units** · Buffer target: 397 · ✅

---

### Daily Journal (Teal) `EIDJ5100`

Peak demand hits in **Dec** at **11,179 units** globally. We have **61,237 units** across all channels against a total need of **52,334** — a surplus of **8,903 units**. No new print is required. However, the stock isn't in the right place: most of it sits in the UK or US hubs while FBA channels are light. The plan repositions hub surplus into FBA and uses small targeted top-up prints where transfer routes are blocked.

#### Monthly Demand Forecast

*Max of 2024 and 2025 actuals taken for each month. The higher year is used so we plan for the strongest Q4 we've seen.*

| Month | 2024 Actual | 2025 Actual | Plan Uses | Cumulative |
| :--- | ---: | ---: | ---: | ---: |
| May 2026 | 3,567 | 1,806 | **3,567** ← 2024 | 3,567 |
| Jun | 2,655 | 1,323 | **2,655** ← 2024 | 6,222 |
| Jul | 4,263 | 1,174 | **4,263** ← 2024 | 10,485 |
| Aug | 4,704 | 906 | **4,704** ← 2024 | 15,189 |
| Sep | 4,556 | 993 | **4,556** ← 2024 | 19,745 |
| Oct | 3,542 | 1,404 | **3,542** ← 2024 | 23,287 |
| Nov | 6,698 | 3,774 | **6,698** ← 2024 | 29,985 |
| Dec | 11,179 | 5,495 | **11,179** ← 2024 | 41,164 |
| Jan 2027 | 3,580 | 4,193 | **4,193** ← 2025 | 45,357 |
| **9-Month Total** | | | | **45,357** |

#### 30-Day Safety Buffer Calculation

*Average of Feb, Mar, Apr monthly sales — one month's worth of stock that must remain at end of January.*

| Month | 2024 | 2025 | Monthly Average |
| :--- | ---: | ---: | ---: |
| Feb | 2,930 | 3,656 | 2,740 |
| Mar | 2,820 | 2,865 | 2,612 |
| Apr | 4,962 | 1,783 | 3,372 |
| **30-day buffer (avg of 3 months)** | | | **2,908** |

> ✅ **Globally sufficient: +8,903 units ahead.** Total stock: 61,237 · Total need: 52,334 · **No print run needed.**

#### US & CA Channel Analysis

**Step 1 — How much hub stock is reserved for Shopify vs available for FBA?**

| | US Hubs (HBG / SLI / SAV / KCM) | CA Hub |
| :--- | ---: | ---: |
| Current stock at hub | 34,801 | 4,526 |
| Shopify demand May–Jan | 13,584 | 2,722 |
| 30-day Shopify buffer | 688 | 207 |
| **Shopify reserve (must keep)** | **14,272** | **2,929** |
| **Hub result** | **+20,529 surplus → transfer 10,185 to US FBA** | **+1,597 surplus → transfer 1,429 to CA FBA** |

**Step 2 — After hub transfer, is FBA covered?**

| | Amazon US FBA | Amazon CA FBA |
| :--- | ---: | ---: |
| Current FBA stock | 1,050 | 0 |
| Transfer in from hub | +10,185 | +1,429 |
| Supplier stock | +0 | +1,500 |
| **Total FBA stock** | **11,235** | **2,929** |
| FBA demand May–Jan | 10,396 | 2,722 |
| 30-day FBA buffer | 839 | 207 |
| **FBA total need** | **11,235** | **2,929** |
| **FBA result** | **✅ Covered (+0)** | **✅ Covered (+0)** |

**US Hub monthly sell-through (US Shopify channel):**

| Month | Projected Sales | Hub Balance | vs Buffer |
| :--- | ---: | ---: | ---: |
| May | 1,359 | **23,257** | +22,569 |
| Jun | 745 | **22,512** | +21,824 |
| Jul | 1,336 | **21,176** | +20,488 |
| Aug | 1,385 | **19,791** | +19,103 |
| Sep | 1,189 | **18,602** | +17,914 |
| Oct | 838 | **17,764** | +17,076 |
| Nov | 1,602 | **16,162** | +15,474 |
| Dec | 3,893 | **12,269** | +11,581 |
| Jan | 1,237 | **11,032** | +10,344 |

> Jan 2027 hub balance: **11,032** · Buffer target: 688 · ✅ Hub ends above buffer

#### Per-Channel Stock Check

| Channel | 9-Mo Demand | 30-Day Buffer | Total Need | Stock on Hand | Decision |
| :--- | ---: | ---: | ---: | ---: | :--- |
| Amazon US FBA | 10,396 | 839 | 11,235 | 1,050 | ✅ Covered by current stock |
| Amazon CA FBA | 2,722 | 207 | 2,929 | 0 | ✅ Covered by current stock + 1,500 supplier |
| US Shopify | 13,584 | 688 | 14,272 | 34,801 (HBG+SLI+SAV+KCM) | Reserve 14,272 for Shopify · Transfer 10,185 surplus → US FBA |
| CA Shopify | 2,722 | 207 | 2,929 | 4,526 (CA hub) | Reserve 2,929 for Shopify · Transfer 1,429 surplus → CA FBA |
| UK | 10,257 | 682 | 10,939 | 12,161 | ✅ Covered by current stock |
| EU | 707 | 61 | 768 | 958 | ✅ Covered by current stock |
| AU | 8,809 | 451 | 9,260 | 6,241 | ✅ Covered by current stock |

> **UK:** 12,161 current − 1,221 transferred = **10,940 remaining** vs UK demand 10,257 · ✅ covered

> **Decision: No full print run.** Hub surplus transfers proceed · Top-up prints: 1,797 to AU

#### Rolling Depletion Forecast

*Starting stock = current inventory after all transfers and print runs arrive. Each month we subtract max projected sales. The goal: end January above the 30-day buffer line.*

#### Amazon US FBA

*Starting stock: **11,235** (1,050 current + + 10,185 transfer)*

*30-day buffer target: **839 units** (Feb avg 748 + Mar avg 905 + Apr avg 865 = 839)*

| Month | Projected Sales | Ending Stock | vs Buffer | Weeks of Cover |
| :--- | ---: | ---: | ---: | ---: |
| May 2026 | 736 | **10,499** | +9,660 | 63.2w |
| Jun | 736 | **9,763** | +8,924 | 40.0w |
| Jul | 1,047 | **8,716** | +7,877 | 42.6w |
| Aug | 907 | **7,809** | +6,970 | 39.8w |
| Sep | 869 | **6,940** | +6,101 | 37.4w |
| Oct | 796 | **6,144** | +5,305 | 19.4w |
| Nov | 1,403 | **4,741** | +3,902 | 7.2w |
| Dec | 2,820 | **1,921** | +1,082 | 7.9w |
| Jan 2027 | 1,082 | **839** | +0 | 13.3w |

> **Jan 2027 ending: 839 units** · Buffer target: 839 · ✅

#### Amazon CA FBA

*Starting stock: **2,929** (0 current + + 2,929 transfer + supplier)*

*30-day buffer target: **207 units** (Feb avg 186 + Mar avg 148 + Apr avg 289 = 207)*

| Month | Projected Sales | Ending Stock | vs Buffer | Weeks of Cover |
| :--- | ---: | ---: | ---: | ---: |
| May 2026 | 473 | **2,456** | +2,249 | 46.5w |
| Jun | 234 | **2,222** | +2,015 | 40.4w |
| Jul | 236 | **1,986** | +1,779 | 43.1w |
| Aug | 204 | **1,782** | +1,575 | 24.1w |
| Sep | 327 | **1,455** | +1,248 | 33.9w |
| Oct | 184 | **1,271** | +1,064 | 15.1w |
| Nov | 372 | **899** | +692 | 8.8w |
| Dec | 436 | **463** | +256 | 8.0w |
| Jan 2027 | 256 | **207** | +0 | 13.3w |

> **Jan 2027 ending: 207 units** · Buffer target: 207 · ✅

#### US Shopify

*Starting stock: **24,615** (HBG+SLI+SAV+KCM: 34,801 − 10,185 transferred to US FBA)*

*30-day buffer target: **688 units** (Feb avg 724 + Mar avg 550 + Apr avg 789 = 688)*

| Month | Projected Sales | Ending Stock | vs Buffer | Weeks of Cover |
| :--- | ---: | ---: | ---: | ---: |
| May 2026 | 1,359 | **23,256** | +22,568 | 138.2w |
| Jun | 745 | **22,511** | +21,823 | 72.2w |
| Jul | 1,336 | **21,175** | +20,487 | 67.7w |
| Aug | 1,385 | **19,790** | +19,102 | 73.7w |
| Sep | 1,189 | **18,601** | +17,913 | 95.1w |
| Oct | 838 | **17,763** | +17,075 | 49.1w |
| Nov | 1,602 | **16,161** | +15,473 | 17.8w |
| Dec | 3,893 | **12,268** | +11,580 | 43.9w |
| Jan 2027 | 1,237 | **11,031** | +10,343 | 213.0w |

> **Jan 2027 ending: 11,031 units** · Buffer target: 688 · ✅

#### CA Shopify

*Starting stock: **3,096** (CA hub: 4,526 − 1,429 to CA FBA)*

*30-day buffer target: **207 units** (Feb avg 186 + Mar avg 148 + Apr avg 289 = 207)*

| Month | Projected Sales | Ending Stock | vs Buffer | Weeks of Cover |
| :--- | ---: | ---: | ---: | ---: |
| May 2026 | 473 | **2,623** | +2,415 | 49.6w |
| Jun | 234 | **2,389** | +2,181 | 43.4w |
| Jul | 236 | **2,153** | +1,945 | 46.7w |
| Aug | 204 | **1,949** | +1,741 | 26.4w |
| Sep | 327 | **1,622** | +1,414 | 37.8w |
| Oct | 184 | **1,438** | +1,230 | 17.1w |
| Nov | 372 | **1,066** | +858 | 10.5w |
| Dec | 436 | **630** | +422 | 10.9w |
| Jan 2027 | 256 | **374** | +166 | 23.9w |

> **Jan 2027 ending: 374 units** · Buffer target: 207 · ✅

#### UK

*Starting stock: **10,939** (12,161 current − 1,221 transferred out)*

*30-day buffer target: **682 units** (Feb avg 606 + Mar avg 554 + Apr avg 885 = 682)*

| Month | Projected Sales | Ending Stock | vs Buffer | Weeks of Cover |
| :--- | ---: | ---: | ---: | ---: |
| May 2026 | 278 | **10,661** | +9,979 | 199.2w |
| Jun | 237 | **10,424** | +9,742 | 78.1w |
| Jul | 572 | **9,852** | +9,170 | 30.4w |
| Aug | 1,433 | **8,419** | +7,737 | 26.4w |
| Sep | 1,414 | **7,005** | +6,323 | 27.7w |
| Oct | 1,082 | **5,923** | +5,241 | 17.0w |
| Nov | 1,543 | **4,380** | +3,698 | 7.9w |
| Dec | 2,373 | **2,007** | +1,325 | 6.7w |
| Jan 2027 | 1,325 | **682** | +0 | 13.3w |

> **Jan 2027 ending: 682 units** · Buffer target: 682 · ✅

#### AU

*Starting stock: **9,260** (6,241 current + + 3,019 top-up print)*

*30-day buffer target: **451 units** (Feb avg 425 + Mar avg 396 + Apr avg 531 = 451)*

| Month | Projected Sales | Ending Stock | vs Buffer | Weeks of Cover |
| :--- | ---: | ---: | ---: | ---: |
| May 2026 | 832 | **8,428** | +7,977 | 45.0w |
| Jun | 829 | **7,599** | +7,148 | 30.4w |
| Jul | 1,072 | **6,527** | +6,076 | 37.3w |
| Aug | 775 | **5,752** | +5,301 | 33.7w |
| Sep | 757 | **4,995** | +4,544 | 33.3w |
| Oct | 642 | **4,353** | +3,902 | 10.8w |
| Nov | 1,778 | **2,575** | +2,124 | 6.7w |
| Dec | 1,657 | **918** | +467 | 8.7w |
| Jan 2027 | 467 | **451** | +0 | 13.3w |

> **Jan 2027 ending: 451 units** · Buffer target: 451 · ✅

---

### Daily Journal (Green) `EIDJ5200`

Peak demand hits in **Dec** at **4,700 units** globally. We have **62,275 units** across all channels against a total need of **23,249** — a surplus of **39,026 units**. No new print is required. However, the stock isn't in the right place: most of it sits in the UK or US hubs while FBA channels are light. The plan repositions hub surplus into FBA and uses small targeted top-up prints where transfer routes are blocked.

#### Monthly Demand Forecast

*Max of 2024 and 2025 actuals taken for each month. The higher year is used so we plan for the strongest Q4 we've seen.*

| Month | 2024 Actual | 2025 Actual | Plan Uses | Cumulative |
| :--- | ---: | ---: | ---: | ---: |
| May 2026 | 2,352 | 839 | **2,352** ← 2024 | 2,352 |
| Jun | 1,466 | 772 | **1,466** ← 2024 | 3,818 |
| Jul | 1,754 | 657 | **1,754** ← 2024 | 5,572 |
| Aug | 1,693 | 449 | **1,693** ← 2024 | 7,265 |
| Sep | 1,446 | 485 | **1,446** ← 2024 | 8,711 |
| Oct | 1,496 | 504 | **1,496** ← 2024 | 10,207 |
| Nov | 3,020 | 2,443 | **3,020** ← 2024 | 13,227 |
| Dec | 4,700 | 2,800 | **4,700** ← 2024 | 17,927 |
| Jan 2027 | 1,322 | 1,987 | **1,987** ← 2025 | 19,914 |
| **9-Month Total** | | | | **19,914** |

#### 30-Day Safety Buffer Calculation

*Average of Feb, Mar, Apr monthly sales — one month's worth of stock that must remain at end of January.*

| Month | 2024 | 2025 | Monthly Average |
| :--- | ---: | ---: | ---: |
| Feb | 942 | 1,657 | 1,088 |
| Mar | 487 | 1,487 | 1,009 |
| Apr | 3,948 | 975 | 2,461 |
| **30-day buffer (avg of 3 months)** | | | **1,519** |

> ✅ **Globally sufficient: +39,026 units ahead.** Total stock: 62,275 · Total need: 23,249 · **No print run needed.**

#### US & CA Channel Analysis

**Step 1 — How much hub stock is reserved for Shopify vs available for FBA?**

| | US Hubs (HBG / SLI / SAV / KCM) | CA Hub |
| :--- | ---: | ---: |
| Current stock at hub | 19,209 | 4,274 |
| Shopify demand May–Jan | 5,310 | 881 |
| 30-day Shopify buffer | 358 | 73 |
| **Shopify reserve (must keep)** | **5,668** | **954** |
| **Hub result** | **+13,541 surplus → transfer 6,078 to US FBA** | **+3,320 surplus → transfer 954 to CA FBA** |

**Step 2 — After hub transfer, is FBA covered?**

| | Amazon US FBA | Amazon CA FBA |
| :--- | ---: | ---: |
| Current FBA stock | 1,118 | 0 |
| Transfer in from hub | +6,078 | +954 |
| **Total FBA stock** | **7,196** | **954** |
| FBA demand May–Jan | 6,758 | 881 |
| 30-day FBA buffer | 438 | 73 |
| **FBA total need** | **7,196** | **954** |
| **FBA result** | **✅ Covered (+0)** | **✅ Covered (+0)** |

**US Hub monthly sell-through (US Shopify channel):**

| Month | Projected Sales | Hub Balance | vs Buffer |
| :--- | ---: | ---: | ---: |
| May | 682 | **12,449** | +12,091 |
| Jun | 280 | **12,169** | +11,811 |
| Jul | 445 | **11,724** | +11,366 |
| Aug | 446 | **11,278** | +10,920 |
| Sep | 350 | **10,928** | +10,570 |
| Oct | 365 | **10,563** | +10,205 |
| Nov | 1,003 | **9,560** | +9,202 |
| Dec | 1,287 | **8,273** | +7,915 |
| Jan | 452 | **7,821** | +7,463 |

> Jan 2027 hub balance: **7,821** · Buffer target: 358 · ✅ Hub ends above buffer

#### Per-Channel Stock Check

| Channel | 9-Mo Demand | 30-Day Buffer | Total Need | Stock on Hand | Decision |
| :--- | ---: | ---: | ---: | ---: | :--- |
| Amazon US FBA | 6,758 | 438 | 7,196 | 1,118 | ✅ Covered by current stock |
| Amazon CA FBA | 881 | 73 | 954 | 0 | ✅ Covered by current stock |
| US Shopify | 5,310 | 358 | 5,668 | 19,209 (HBG+SLI+SAV+KCM) | Reserve 5,668 for Shopify · Transfer 6,078 surplus → US FBA |
| CA Shopify | 881 | 73 | 954 | 4,274 (CA hub) | Reserve 954 for Shopify · Transfer 954 surplus → CA FBA |
| UK | 4,518 | 480 | 4,998 | 31,861 | ✅ Covered by current stock |
| EU | 559 | 23 | 582 | 938 | ✅ Covered by current stock |
| AU | 2,741 | 155 | 2,896 | 4,875 | ✅ Covered by current stock |
> **Decision: No print run.** Transfers: 6,078 US Hub→Amazon US FBA; 954 CA Hub→Amazon CA FBA

#### Rolling Depletion Forecast

*Starting stock = current inventory after all transfers and print runs arrive. Each month we subtract max projected sales. The goal: end January above the 30-day buffer line.*

#### Amazon US FBA

*Starting stock: **7,196** (1,118 current + + 6,078 transfer)*

*30-day buffer target: **438 units** (Feb avg 378 + Mar avg 470 + Apr avg 465 = 438)*

| Month | Projected Sales | Ending Stock | vs Buffer | Weeks of Cover |
| :--- | ---: | ---: | ---: | ---: |
| May 2026 | 486 | **6,710** | +6,272 | 44.1w |
| Jun | 674 | **6,036** | +5,598 | 41.2w |
| Jul | 628 | **5,408** | +4,970 | 38.0w |
| Aug | 630 | **4,778** | +4,340 | 46.9w |
| Sep | 451 | **4,327** | +3,889 | 34.8w |
| Oct | 533 | **3,794** | +3,356 | 17.6w |
| Nov | 952 | **2,842** | +2,404 | 6.4w |
| Dec | 1,899 | **943** | +505 | 8.3w |
| Jan 2027 | 505 | **438** | +0 | 13.3w |

> **Jan 2027 ending: 438 units** · Buffer target: 438 · ✅

#### Amazon CA FBA

*Starting stock: **954** (0 current + + 954 transfer)*

*30-day buffer target: **73 units** (Feb avg 58 + Mar avg 43 + Apr avg 117 = 73)*

| Month | Projected Sales | Ending Stock | vs Buffer | Weeks of Cover |
| :--- | ---: | ---: | ---: | ---: |
| May 2026 | 133 | **821** | +748 | 50.5w |
| Jun | 72 | **749** | +676 | 42.8w |
| Jul | 75 | **674** | +601 | 45.2w |
| Aug | 66 | **608** | +535 | 35.9w |
| Sep | 75 | **533** | +460 | 38.7w |
| Oct | 59 | **474** | +401 | 13.8w |
| Nov | 152 | **322** | +249 | 9.2w |
| Dec | 150 | **172** | +99 | 7.7w |
| Jan 2027 | 99 | **73** | +0 | 13.3w |

> **Jan 2027 ending: 73 units** · Buffer target: 73 · ✅

#### US Shopify

*Starting stock: **13,131** (HBG+SLI+SAV+KCM: 19,209 − 6,078 transferred to US FBA)*

*30-day buffer target: **358 units** (Feb avg 280 + Mar avg 193 + Apr avg 600 = 358)*

| Month | Projected Sales | Ending Stock | vs Buffer | Weeks of Cover |
| :--- | ---: | ---: | ---: | ---: |
| May 2026 | 682 | **12,449** | +12,091 | 196.9w |
| Jun | 280 | **12,169** | +11,811 | 117.2w |
| Jul | 445 | **11,724** | +11,366 | 116.4w |
| Aug | 446 | **11,278** | +10,920 | 142.7w |
| Sep | 350 | **10,928** | +10,570 | 128.3w |
| Oct | 365 | **10,563** | +10,205 | 46.6w |
| Nov | 1,003 | **9,560** | +9,202 | 31.8w |
| Dec | 1,287 | **8,273** | +7,915 | 81.1w |
| Jan 2027 | 452 | **7,821** | +7,463 | 290.2w |

> **Jan 2027 ending: 7,821 units** · Buffer target: 358 · ✅

#### CA Shopify

*Starting stock: **3,320** (CA hub: 4,274 − 954 to CA FBA)*

*30-day buffer target: **73 units** (Feb avg 58 + Mar avg 43 + Apr avg 117 = 73)*

| Month | Projected Sales | Ending Stock | vs Buffer | Weeks of Cover |
| :--- | ---: | ---: | ---: | ---: |
| May 2026 | 133 | **3,187** | +3,114 | 196.0w |
| Jun | 72 | **3,115** | +3,042 | 178.0w |
| Jul | 75 | **3,040** | +2,967 | 204.0w |
| Aug | 66 | **2,974** | +2,901 | 175.6w |
| Sep | 75 | **2,899** | +2,826 | 210.6w |
| Oct | 59 | **2,840** | +2,767 | 82.7w |
| Nov | 152 | **2,688** | +2,615 | 76.8w |
| Dec | 150 | **2,538** | +2,465 | 113.5w |
| Jan 2027 | 99 | **2,439** | +2,366 | 443.9w |

> **Jan 2027 ending: 2,439 units** · Buffer target: 73 · ✅

#### UK

*Starting stock: **31,861** (31,861 current)*

*30-day buffer target: **480 units** (Feb avg 225 + Mar avg 153 + Apr avg 1,063 = 480)*

| Month | Projected Sales | Ending Stock | vs Buffer | Weeks of Cover |
| :--- | ---: | ---: | ---: | ---: |
| May 2026 | 763 | **31,098** | +30,618 | 706.3w |
| Jun | 195 | **30,903** | +30,423 | 521.4w |
| Jul | 254 | **30,649** | +30,169 | 424.2w |
| Aug | 320 | **30,329** | +29,849 | 386.0w |
| Sep | 348 | **29,981** | +29,501 | 367.1w |
| Oct | 350 | **29,631** | +29,151 | 214.8w |
| Nov | 611 | **29,020** | +28,540 | 125.1w |
| Dec | 994 | **28,026** | +27,546 | 181.7w |
| Jan 2027 | 683 | **27,343** | +26,863 | 756.1w |

> **Jan 2027 ending: 27,343 units** · Buffer target: 480 · ✅

#### AU

*Starting stock: **4,875** (4,875 current)*

*30-day buffer target: **155 units** (Feb avg 126 + Mar avg 128 + Apr avg 210 = 155)*

| Month | Projected Sales | Ending Stock | vs Buffer | Weeks of Cover |
| :--- | ---: | ---: | ---: | ---: |
| May 2026 | 288 | **4,587** | +4,432 | 82.9w |
| Jun | 245 | **4,342** | +4,187 | 52.9w |
| Jul | 352 | **3,990** | +3,835 | 76.5w |
| Aug | 231 | **3,759** | +3,604 | 75.0w |
| Sep | 222 | **3,537** | +3,382 | 80.2w |
| Oct | 189 | **3,348** | +3,193 | 24.3w |
| Nov | 611 | **2,737** | +2,582 | 28.5w |
| Dec | 412 | **2,325** | +2,170 | 53.9w |
| Jan 2027 | 191 | **2,134** | +1,979 | 182.8w |

> **Jan 2027 ending: 2,134 units** · Buffer target: 155 · ✅

---

### Adult Journal `EIDJ5000`

Peak demand hits in **Dec** at **9,070 units** globally. We have **63,514 units** across all channels against a total need of **28,074** — a surplus of **35,440 units**. No new print is required. However, the stock isn't in the right place: most of it sits in the UK or US hubs while FBA channels are light. The plan repositions hub surplus into FBA and uses small targeted top-up prints where transfer routes are blocked.

#### Monthly Demand Forecast

*Max of 2024 and 2025 actuals taken for each month. The higher year is used so we plan for the strongest Q4 we've seen.*

| Month | 2024 Actual | 2025 Actual | Plan Uses | Cumulative |
| :--- | ---: | ---: | ---: | ---: |
| May 2026 | 1,949 | 537 | **1,949** ← 2024 | 1,949 |
| Jun | 1,622 | 627 | **1,622** ← 2024 | 3,571 |
| Jul | 1,365 | 688 | **1,365** ← 2024 | 4,936 |
| Aug | 1,016 | 496 | **1,016** ← 2024 | 5,952 |
| Sep | 864 | 312 | **864** ← 2024 | 6,816 |
| Oct | 806 | 670 | **806** ← 2024 | 7,622 |
| Nov | 3,545 | 1,718 | **3,545** ← 2024 | 11,167 |
| Dec | 9,070 | 1,975 | **9,070** ← 2024 | 20,237 |
| Jan 2027 | 3,663 | 2,418 | **3,663** ← 2024 | 23,900 |
| **9-Month Total** | | | | **23,900** |

#### 30-Day Safety Buffer Calculation

*Average of Feb, Mar, Apr monthly sales — one month's worth of stock that must remain at end of January.*

| Month | 2024 | 2025 | Monthly Average |
| :--- | ---: | ---: | ---: |
| Feb | 1,256 | 1,051 | 985 |
| Mar | 1,081 | 695 | 846 |
| Apr | 1,357 | 421 | 889 |
| **30-day buffer (avg of 3 months)** | | | **906** |

> ✅ **Globally sufficient: +35,440 units ahead.** Total stock: 63,514 · Total need: 28,074 · **No print run needed.**

#### US & CA Channel Analysis

**Step 1 — How much hub stock is reserved for Shopify vs available for FBA?**

| | US Hubs (HBG / SLI / SAV / KCM) | CA Hub |
| :--- | ---: | ---: |
| Current stock at hub | 37,634 | 4,929 |
| Shopify demand May–Jan | 7,093 | 1,589 |
| 30-day Shopify buffer | 156 | 73 |
| **Shopify reserve (must keep)** | **7,249** | **1,662** |
| **Hub result** | **+30,385 surplus → transfer 5,302 to US FBA** | **+3,267 surplus → transfer 190 to CA FBA** |

**Step 2 — After hub transfer, is FBA covered?**

| | Amazon US FBA | Amazon CA FBA |
| :--- | ---: | ---: |
| Current FBA stock | 415 | 0 |
| Transfer in from hub | +5,302 | +190 |
| Supplier stock | +0 | +1,472 |
| **Total FBA stock** | **5,717** | **1,662** |
| FBA demand May–Jan | 5,428 | 1,589 |
| 30-day FBA buffer | 289 | 73 |
| **FBA total need** | **5,717** | **1,662** |
| **FBA result** | **✅ Covered (+0)** | **✅ Covered (+0)** |

**US Hub monthly sell-through (US Shopify channel):**

| Month | Projected Sales | Hub Balance | vs Buffer |
| :--- | ---: | ---: | ---: |
| May | 465 | **31,867** | +31,711 |
| Jun | 369 | **31,498** | +31,342 |
| Jul | 331 | **31,167** | +31,011 |
| Aug | 199 | **30,968** | +30,812 |
| Sep | 163 | **30,805** | +30,649 |
| Oct | 324 | **30,481** | +30,325 |
| Nov | 901 | **29,580** | +29,424 |
| Dec | 2,854 | **26,726** | +26,570 |
| Jan | 1,487 | **25,239** | +25,083 |

> Jan 2027 hub balance: **25,239** · Buffer target: 156 · ✅ Hub ends above buffer

#### Per-Channel Stock Check

| Channel | 9-Mo Demand | 30-Day Buffer | Total Need | Stock on Hand | Decision |
| :--- | ---: | ---: | ---: | ---: | :--- |
| Amazon US FBA | 5,428 | 289 | 5,717 | 415 | ✅ Covered by current stock |
| Amazon CA FBA | 1,589 | 73 | 1,662 | 0 | ✅ Covered by current stock + 1,472 supplier |
| US Shopify | 7,093 | 156 | 7,249 | 37,634 (HBG+SLI+SAV+KCM) | Reserve 7,249 for Shopify · Transfer 5,302 surplus → US FBA |
| CA Shopify | 1,589 | 73 | 1,662 | 4,929 (CA hub) | Reserve 1,662 for Shopify · Transfer 190 surplus → CA FBA |
| UK | 6,892 | 224 | 7,116 | 11,565 | ✅ Covered by current stock |
| EU | 677 | 30 | 707 | 1,599 | ✅ Covered by current stock |
| AU | 3,815 | 142 | 3,957 | 5,900 | ✅ Covered by current stock |
> **Decision: No print run.** Transfers: 5,302 US Hub→Amazon US FBA; 190 CA Hub→Amazon CA FBA

#### Rolling Depletion Forecast

*Starting stock = current inventory after all transfers and print runs arrive. Each month we subtract max projected sales. The goal: end January above the 30-day buffer line.*

#### Amazon US FBA

*Starting stock: **5,717** (415 current + + 5,302 transfer)*

*30-day buffer target: **289 units** (Feb avg 277 + Mar avg 257 + Apr avg 334 = 289)*

| Month | Projected Sales | Ending Stock | vs Buffer | Weeks of Cover |
| :--- | ---: | ---: | ---: | ---: |
| May 2026 | 589 | **5,128** | +4,839 | 54.1w |
| Jun | 420 | **4,708** | +4,419 | 47.0w |
| Jul | 429 | **4,279** | +3,990 | 49.0w |
| Aug | 387 | **3,892** | +3,603 | 55.1w |
| Sep | 313 | **3,579** | +3,290 | 56.2w |
| Oct | 273 | **3,306** | +3,017 | 29.0w |
| Nov | 505 | **2,801** | +2,512 | 7.5w |
| Dec | 1,607 | **1,194** | +905 | 5.8w |
| Jan 2027 | 905 | **289** | +0 | 13.3w |

> **Jan 2027 ending: 289 units** · Buffer target: 289 · ✅

#### Amazon CA FBA

*Starting stock: **1,662** (0 current + + 1,662 transfer + supplier)*

*30-day buffer target: **73 units** (Feb avg 103 + Mar avg 80 + Apr avg 38 = 73)*

| Month | Projected Sales | Ending Stock | vs Buffer | Weeks of Cover |
| :--- | ---: | ---: | ---: | ---: |
| May 2026 | 89 | **1,573** | +1,500 | 84.0w |
| Jun | 83 | **1,490** | +1,417 | 87.5w |
| Jul | 73 | **1,417** | +1,344 | 196.2w |
| Aug | 32 | **1,385** | +1,312 | 107.7w |
| Sep | 57 | **1,328** | +1,255 | 146.0w |
| Oct | 39 | **1,289** | +1,216 | 16.3w |
| Nov | 350 | **939** | +866 | 11.0w |
| Dec | 366 | **573** | +500 | 5.1w |
| Jan 2027 | 500 | **73** | +0 | 13.3w |

> **Jan 2027 ending: 73 units** · Buffer target: 73 · ✅

#### US Shopify

*Starting stock: **32,331** (HBG+SLI+SAV+KCM: 37,634 − 5,302 transferred to US FBA)*

*30-day buffer target: **156 units** (Feb avg 203 + Mar avg 129 + Apr avg 136 = 156)*

| Month | Projected Sales | Ending Stock | vs Buffer | Weeks of Cover |
| :--- | ---: | ---: | ---: | ---: |
| May 2026 | 465 | **31,866** | +31,710 | 382.4w |
| Jun | 369 | **31,497** | +31,341 | 407.8w |
| Jul | 331 | **31,166** | +31,010 | 693.6w |
| Aug | 199 | **30,967** | +30,811 | 841.4w |
| Sep | 163 | **30,804** | +30,648 | 407.5w |
| Oct | 324 | **30,480** | +30,324 | 149.8w |
| Nov | 901 | **29,579** | +29,423 | 44.4w |
| Dec | 2,854 | **26,725** | +26,569 | 79.6w |
| Jan 2027 | 1,487 | **25,238** | +25,082 | 2146.4w |

> **Jan 2027 ending: 25,238 units** · Buffer target: 156 · ✅

#### CA Shopify

*Starting stock: **4,738** (CA hub: 4,929 − 190 to CA FBA)*

*30-day buffer target: **73 units** (Feb avg 103 + Mar avg 80 + Apr avg 38 = 73)*

| Month | Projected Sales | Ending Stock | vs Buffer | Weeks of Cover |
| :--- | ---: | ---: | ---: | ---: |
| May 2026 | 89 | **4,649** | +4,575 | 248.1w |
| Jun | 83 | **4,566** | +4,492 | 268.1w |
| Jul | 73 | **4,493** | +4,419 | 621.8w |
| Aug | 32 | **4,461** | +4,387 | 346.6w |
| Sep | 57 | **4,404** | +4,330 | 484.0w |
| Oct | 39 | **4,365** | +4,291 | 55.2w |
| Nov | 350 | **4,015** | +3,941 | 47.0w |
| Dec | 366 | **3,649** | +3,575 | 32.3w |
| Jan 2027 | 500 | **3,149** | +3,075 | 566.2w |

> **Jan 2027 ending: 3,149 units** · Buffer target: 73 · ✅

#### UK

*Starting stock: **11,565** (11,565 current)*

*30-day buffer target: **224 units** (Feb avg 217 + Mar avg 204 + Apr avg 252 = 224)*

| Month | Projected Sales | Ending Stock | vs Buffer | Weeks of Cover |
| :--- | ---: | ---: | ---: | ---: |
| May 2026 | 547 | **11,018** | +10,793 | 93.1w |
| Jun | 524 | **10,494** | +10,269 | 139.2w |
| Jul | 323 | **10,171** | +9,946 | 173.9w |
| Aug | 259 | **9,912** | +9,687 | 236.0w |
| Sep | 186 | **9,726** | +9,501 | 132.3w |
| Oct | 315 | **9,411** | +9,186 | 36.7w |
| Nov | 1,135 | **8,276** | +8,051 | 12.5w |
| Dec | 2,842 | **5,434** | +5,209 | 31.6w |
| Jan 2027 | 761 | **4,673** | +4,448 | 276.0w |

> **Jan 2027 ending: 4,673 units** · Buffer target: 224 · ✅

#### AU

*Starting stock: **5,900** (5,900 current)*

*30-day buffer target: **142 units** (Feb avg 157 + Mar avg 144 + Apr avg 125 = 142)*

| Month | Projected Sales | Ending Stock | vs Buffer | Weeks of Cover |
| :--- | ---: | ---: | ---: | ---: |
| May 2026 | 259 | **5,641** | +5,499 | 110.5w |
| Jun | 226 | **5,415** | +5,273 | 111.0w |
| Jul | 209 | **5,206** | +5,064 | 155.8w |
| Aug | 148 | **5,058** | +4,916 | 154.5w |
| Sep | 145 | **4,913** | +4,771 | 181.5w |
| Oct | 116 | **4,797** | +4,655 | 32.5w |
| Nov | 654 | **4,143** | +4,001 | 12.7w |
| Dec | 1,401 | **2,742** | +2,600 | 18.5w |
| Jan 2027 | 657 | **2,085** | +1,943 | 194.6w |

> **Jan 2027 ending: 2,085 units** · Buffer target: 142 · ✅

---

### Dream Affirmation Cards `EIDC2101`

Peak demand hits in **Nov** at **7,489 units** globally. We have **43,091 units** across all channels against a total need of **31,344** — a surplus of **11,747 units**. No new print is required. However, the stock isn't in the right place: most of it sits in the UK or US hubs while FBA channels are light. The plan repositions hub surplus into FBA and uses small targeted top-up prints where transfer routes are blocked.

#### Monthly Demand Forecast

*Max of 2024 and 2025 actuals taken for each month. The higher year is used so we plan for the strongest Q4 we've seen.*

| Month | 2024 Actual | 2025 Actual | Plan Uses | Cumulative |
| :--- | ---: | ---: | ---: | ---: |
| May 2026 | — | 1,079 | **1,079** ← 2025 | 1,079 |
| Jun | — | 532 | **532** ← 2025 | 1,611 |
| Jul | — | 614 | **614** ← 2025 | 2,225 |
| Aug | — | 463 | **463** ← 2025 | 2,688 |
| Sep | 3,761 | 326 | **3,761** ← 2024 | 6,449 |
| Oct | 3,943 | 584 | **3,943** ← 2024 | 10,392 |
| Nov | 3,884 | 7,489 | **7,489** ← 2025 | 17,881 |
| Dec | 6,648 | 5,140 | **6,648** ← 2024 | 24,529 |
| Jan 2027 | — | 2,056 | **2,056** ← 2025 | 26,585 |
| **9-Month Total** | | | | **26,585** |

#### 30-Day Safety Buffer Calculation

*Average of Feb, Mar, Apr monthly sales — one month's worth of stock that must remain at end of January.*

| Month | 2024 | 2025 | Monthly Average |
| :--- | ---: | ---: | ---: |
| Feb | — | 1,159 | 975 |
| Mar | — | 958 | 864 |
| Apr | — | 2,459 | 2,459 |
| **30-day buffer (avg of 3 months)** | | | **1,432** |

> ✅ **Globally sufficient: +11,747 units ahead.** Total stock: 43,091 · Total need: 31,344 · **No print run needed.**

#### US & CA Channel Analysis

**Step 1 — How much hub stock is reserved for Shopify vs available for FBA?**

| | US Hubs (HBG / SLI / SAV / KCM) | CA Hub |
| :--- | ---: | ---: |
| Current stock at hub | 25,105 | 2,474 |
| Shopify demand May–Jan | 10,369 | 1,896 |
| 30-day Shopify buffer | 582 | 137 |
| **Shopify reserve (must keep)** | **10,951** | **2,033** |
| **Hub result** | **+14,154 surplus → transfer 3,514 to US FBA** | **+441 surplus → transfer 440 to CA FBA** |

**Step 2 — After hub transfer, is FBA covered?**

| | Amazon US FBA | Amazon CA FBA |
| :--- | ---: | ---: |
| Current FBA stock | 794 | 0 |
| Transfer in from hub | +3,514 | +440 |
| **Total FBA stock** | **4,308** | **440** |
| FBA demand May–Jan | 3,957 | 1,896 |
| 30-day FBA buffer | 351 | 137 |
| **FBA total need** | **4,308** | **2,033** |
| **FBA result** | **✅ Covered (+0)** | **−1,593 still short** |

**US Hub monthly sell-through (US Shopify channel):**

| Month | Projected Sales | Hub Balance | vs Buffer |
| :--- | ---: | ---: | ---: |
| May | 276 | **21,315** | +20,733 |
| Jun | 109 | **21,206** | +20,624 |
| Jul | 265 | **20,941** | +20,359 |
| Aug | 160 | **20,781** | +20,199 |
| Sep | 2,103 | **18,678** | +18,096 |
| Oct | 2,157 | **16,521** | +15,939 |
| Nov | 1,599 | **14,922** | +14,340 |
| Dec | 2,854 | **12,068** | +11,486 |
| Jan | 846 | **11,222** | +10,640 |

> Jan 2027 hub balance: **11,222** · Buffer target: 582 · ✅ Hub ends above buffer

#### Per-Channel Stock Check

| Channel | 9-Mo Demand | 30-Day Buffer | Total Need | Stock on Hand | Decision |
| :--- | ---: | ---: | ---: | ---: | :--- |
| Amazon US FBA | 3,957 | 351 | 4,308 | 794 | ✅ Covered by current stock |
| Amazon CA FBA | 1,896 | 137 | 2,033 | 0 | ✅ Covered by current stock |
| US Shopify | 10,369 | 582 | 10,951 | 25,105 (HBG+SLI+SAV+KCM) | Reserve 10,951 for Shopify · Transfer 3,514 surplus → US FBA |
| CA Shopify | 1,896 | 137 | 2,033 | 2,474 (CA hub) | Reserve 2,033 for Shopify · Transfer 440 surplus → CA FBA |
| UK | 6,116 | 108 | 6,224 | 9,234 | ✅ Covered by current stock |
| EU | 1,408 | 58 | 1,466 | 943 | ✅ Covered by current stock |
| AU | 4,132 | 194 | 4,326 | 4,541 | ✅ Covered by current stock |

> **UK:** 9,234 current − 523 transferred = **8,711 remaining** vs UK demand 6,116 · ✅ covered

> **Decision: No full print run.** Hub surplus transfers proceed · Top-up prints: 1,593 to Amazon CA FBA

#### Rolling Depletion Forecast

*Starting stock = current inventory after all transfers and print runs arrive. Each month we subtract max projected sales. The goal: end January above the 30-day buffer line.*

#### Amazon US FBA

*Starting stock: **4,308** (794 current + + 3,514 transfer)*

*30-day buffer target: **351 units** (Feb avg 263 + Mar avg 306 + Apr avg 485 = 351)*

| Month | Projected Sales | Ending Stock | vs Buffer | Weeks of Cover |
| :--- | ---: | ---: | ---: | ---: |
| May 2026 | 296 | **4,012** | +3,661 | 109.7w |
| Jun | 162 | **3,850** | +3,499 | 109.3w |
| Jul | 151 | **3,699** | +3,348 | 89.5w |
| Aug | 183 | **3,516** | +3,165 | 88.5w |
| Sep | 176 | **3,340** | +2,989 | 58.2w |
| Oct | 246 | **3,094** | +2,743 | 12.6w |
| Nov | 1,091 | **2,003** | +1,652 | 7.1w |
| Dec | 1,206 | **797** | +446 | 7.9w |
| Jan 2027 | 446 | **351** | +0 | 13.3w |

> **Jan 2027 ending: 351 units** · Buffer target: 351 · ✅

#### Amazon CA FBA

*Starting stock: **2,033** (0 current + + 2,033 top-up print)*

*30-day buffer target: **137 units** (Feb avg 83 + Mar avg 59 + Apr avg 270 = 137)*

| Month | Projected Sales | Ending Stock | vs Buffer | Weeks of Cover |
| :--- | ---: | ---: | ---: | ---: |
| May 2026 | 119 | **1,914** | +1,776 | 302.8w |
| Jun | 28 | **1,886** | +1,748 | 351.5w |
| Jul | 23 | **1,863** | +1,726 | 250.1w |
| Aug | 33 | **1,830** | +1,692 | 21.6w |
| Sep | 376 | **1,454** | +1,316 | 18.0w |
| Oct | 346 | **1,108** | +970 | 9.8w |
| Nov | 503 | **605** | +468 | 7.6w |
| Dec | 341 | **264** | +126 | 9.2w |
| Jan 2027 | 127 | **137** | +0 | 13.3w |

> **Jan 2027 ending: 137 units** · Buffer target: 137 · ✅

#### US Shopify

*Starting stock: **21,590** (HBG+SLI+SAV+KCM: 25,105 − 3,514 transferred to US FBA)*

*30-day buffer target: **582 units** (Feb avg 295 + Mar avg 245 + Apr avg 1,207 = 582)*

| Month | Projected Sales | Ending Stock | vs Buffer | Weeks of Cover |
| :--- | ---: | ---: | ---: | ---: |
| May 2026 | 276 | **21,314** | +20,733 | 866.0w |
| Jun | 109 | **21,205** | +20,624 | 342.9w |
| Jul | 265 | **20,940** | +20,359 | 579.6w |
| Aug | 160 | **20,780** | +20,199 | 43.8w |
| Sep | 2,103 | **18,677** | +18,096 | 37.1w |
| Oct | 2,157 | **16,520** | +15,939 | 45.8w |
| Nov | 1,599 | **14,921** | +14,340 | 22.4w |
| Dec | 2,854 | **12,067** | +11,486 | 63.2w |
| Jan 2027 | 846 | **11,221** | +10,640 | 255.9w |

> **Jan 2027 ending: 11,221 units** · Buffer target: 582 · ✅

#### CA Shopify

*Starting stock: **2,033** (CA hub: 2,474 − 440 to CA FBA)*

*30-day buffer target: **137 units** (Feb avg 83 + Mar avg 59 + Apr avg 270 = 137)*

| Month | Projected Sales | Ending Stock | vs Buffer | Weeks of Cover |
| :--- | ---: | ---: | ---: | ---: |
| May 2026 | 119 | **1,914** | +1,776 | 302.8w |
| Jun | 28 | **1,886** | +1,748 | 351.5w |
| Jul | 23 | **1,863** | +1,726 | 250.1w |
| Aug | 33 | **1,830** | +1,692 | 21.6w |
| Sep | 376 | **1,454** | +1,316 | 18.0w |
| Oct | 346 | **1,108** | +970 | 9.8w |
| Nov | 503 | **605** | +468 | 7.6w |
| Dec | 341 | **264** | +126 | 9.2w |
| Jan 2027 | 127 | **137** | +0 | 13.3w |

> **Jan 2027 ending: 137 units** · Buffer target: 137 · ✅

#### UK

*Starting stock: **8,710** (9,234 current − 523 transferred out)*

*30-day buffer target: **108 units** (Feb avg 114 + Mar avg 70 + Apr avg 140 = 108)*

| Month | Projected Sales | Ending Stock | vs Buffer | Weeks of Cover |
| :--- | ---: | ---: | ---: | ---: |
| May 2026 | 117 | **8,593** | +8,486 | 295.0w |
| Jun | 129 | **8,464** | +8,356 | 863.7w |
| Jul | 42 | **8,422** | +8,314 | 2664.3w |
| Aug | 14 | **8,408** | +8,300 | 55.7w |
| Sep | 669 | **7,739** | +7,632 | 58.9w |
| Oct | 563 | **7,176** | +7,068 | 13.5w |
| Nov | 2,346 | **4,830** | +4,722 | 11.5w |
| Dec | 1,794 | **3,036** | +2,928 | 30.4w |
| Jan 2027 | 442 | **2,594** | +2,486 | 318.7w |

> **Jan 2027 ending: 2,594 units** · Buffer target: 108 · ✅

#### AU

*Starting stock: **4,541** (4,541 current)*

*30-day buffer target: **194 units** (Feb avg 169 + Mar avg 140 + Apr avg 275 = 194)*

| Month | Projected Sales | Ending Stock | vs Buffer | Weeks of Cover |
| :--- | ---: | ---: | ---: | ---: |
| May 2026 | 185 | **4,356** | +4,161 | 241.1w |
| Jun | 80 | **4,276** | +4,081 | 192.9w |
| Jul | 95 | **4,181** | +3,986 | 280.5w |
| Aug | 66 | **4,115** | +3,920 | 29.7w |
| Sep | 613 | **3,502** | +3,307 | 21.0w |
| Oct | 716 | **2,786** | +2,591 | 8.9w |
| Nov | 1,381 | **1,405** | +1,210 | 8.1w |
| Dec | 746 | **659** | +464 | 11.7w |
| Jan 2027 | 250 | **409** | +214 | 27.9w |

> **Jan 2027 ending: 409 units** · Buffer target: 194 · ✅

---

### Know Me If You Can Cards `EIDJB5002`

Peak demand hits in **Nov** at **14,995 units** globally. Even counting all warehouses — hubs, FBA, UK, AU — we're **5,805 units short** of covering every channel through January with a 30-day buffer (34,177 available vs 39,982 needed). Without a print run, **Amazon CA FBA** runs out of stock in **Jun**, right before or during peak season. New stock is ordered and shipped direct from the factory to each short channel.

#### Monthly Demand Forecast

*Max of 2024 and 2025 actuals taken for each month. The higher year is used so we plan for the strongest Q4 we've seen.*

| Month | 2024 Actual | 2025 Actual | Plan Uses | Cumulative |
| :--- | ---: | ---: | ---: | ---: |
| May 2026 | — | — | — | 0 |
| Jun | — | 1,926 | **1,926** ← 2025 | 1,926 |
| Jul | — | 431 | **431** ← 2025 | 2,357 |
| Aug | — | 1,234 | **1,234** ← 2025 | 3,591 |
| Sep | — | 1,425 | **1,425** ← 2025 | 5,016 |
| Oct | — | 1,574 | **1,574** ← 2025 | 6,590 |
| Nov | — | 14,995 | **14,995** ← 2025 | 21,585 |
| Dec | — | 12,518 | **12,518** ← 2025 | 34,103 |
| Jan 2027 | — | — | — | 34,103 |
| **9-Month Total** | | | | **34,103** |

#### 30-Day Safety Buffer Calculation

*Average of Feb, Mar, Apr monthly sales — one month's worth of stock that must remain at end of January.*

| Month | 2024 | 2025 | Monthly Average |
| :--- | ---: | ---: | ---: |
| Feb | — | — | 981 |
| Mar | — | — | 844 |
| Apr | — | — | 0 |
| **30-day buffer (avg of 3 months)** | | | **608** |

> ⚠️ **Globally short: 5,805 units.** Total stock across all channels: 34,177 · Total need (demand + buffer): 39,982 · **Print run triggered.**

#### US & CA Channel Analysis

**Step 1 — How much hub stock is reserved for Shopify vs available for FBA?**

| | US Hubs (HBG / SLI / SAV / KCM) | CA Hub |
| :--- | ---: | ---: |
| Current stock at hub | 15,083 | 1,547 |
| Shopify demand May–Jan | 14,007 | 2,591 |
| 30-day Shopify buffer | 149 | 51 |
| **Shopify reserve (must keep)** | **14,156** | **2,642** |
| **Hub result** | **+927 surplus → transfer 0 to US FBA** | **−1,095 shortfall — hub needs 1,095 more** |

**Step 2 — After hub transfer, is FBA covered?**

| | Amazon US FBA | Amazon CA FBA |
| :--- | ---: | ---: |
| Current FBA stock | 7,426 | 0 |
| Supplier stock | +2,077 | +0 |
| **Total FBA stock** | **9,503** | **0** |
| FBA demand May–Jan | 9,287 | 2,591 |
| 30-day FBA buffer | 216 | 51 |
| **FBA total need** | **9,503** | **2,642** |
| **FBA result** | **✅ Covered (+0)** | **−2,642 still short** |

**US Hub monthly sell-through (US Shopify channel):**

| Month | Projected Sales | Hub Balance | vs Buffer |
| :--- | ---: | ---: | ---: |
| May | 0 | **15,083** | +14,934 |
| Jun | 1,130 | **13,953** | +13,804 |
| Jul | 229 | **13,724** | +13,575 |
| Aug | 551 | **13,173** | +13,024 |
| Sep | 846 | **12,327** | +12,178 |
| Oct | 702 | **11,625** | +11,476 |
| Nov | 5,188 | **6,437** | +6,288 |
| Dec | 4,530 | **1,907** | +1,758 |
| Jan | 831 | **1,076** | +927 |

> Jan 2027 hub balance: **1,076** · Buffer target: 149 · ✅ Hub ends above buffer

#### Per-Channel Stock Check

| Channel | 9-Mo Demand | 30-Day Buffer | Total Need | Stock on Hand | Decision |
| :--- | ---: | ---: | ---: | ---: | :--- |
| Amazon US FBA | 9,287 | 216 | 9,503 | 7,426 | ✅ Covered by current stock + 2,077 supplier |
| Amazon CA FBA | 2,591 | 51 | 2,642 | 0 | 🖨️ Print 2,642 direct |
| US Shopify | 14,007 | 149 | 14,156 | 15,083 (HBG+SLI+SAV+KCM) | ✅ Covered by current stock |
| CA Shopify | 2,591 | 51 | 2,642 | 1,547 (CA hub) | 🖨️ Print 1,095 direct |
| UK | 3,908 | 67 | 3,975 | 3,368 | 🖨️ Print 607 direct |
| EU | 672 | 44 | 716 | 681 | 🖨️ Print 35 direct |
| AU | 6,267 | 78 | 6,345 | 3,128 | 📦 866 supplier, 🖨️ Print 2,351 direct (UK→AU blocked for cards) |
> **Decision: Print 6,730 units** (2,642 to Amazon CA FBA + 1,095 to CA Shopify + 607 to UK + 35 to EU + 2,351 to AU) · ship direct from factory

**Without this print run — what would break:**

| Channel | Stock Without Print | Print Adds | Consequence |
| :--- | ---: | ---: | :--- |
| Amazon CA FBA | 0 | 2,642 | 🚨 Runs out of stock in **Jun** |
| AU | 3,994 | 2,351 | 🚨 Runs out of stock in **Nov** |
| CA Shopify | 1,547 | 1,095 | 🚨 Runs out of stock in **Nov** |
| UK | 3,368 | 607 | 🚨 Runs out of stock in **Dec** |
| EU | 681 | 35 | ⚠️ Falls below 30-day safety buffer in **Dec** |

#### Rolling Depletion Forecast

*Starting stock = current inventory after all transfers and print runs arrive. Each month we subtract max projected sales. The goal: end January above the 30-day buffer line.*

#### Amazon US FBA

*Starting stock: **9,503** (7,426 current + + 2,077 supplier)*

*30-day buffer target: **216 units** (Feb avg 333 + Mar avg 317 = 216)*

| Month | Projected Sales | Ending Stock | vs Buffer | Weeks of Cover |
| :--- | ---: | ---: | ---: | ---: |
| May 2026 | 0 | **9,503** | +9,287 | — |
| Jun | 0 | **9,503** | +9,287 | — |
| Jul | 0 | **9,503** | +9,287 | — |
| Aug | 0 | **9,503** | +9,287 | 876.8w |
| Sep | 48 | **9,455** | +9,239 | 232.9w |
| Oct | 174 | **9,281** | +9,065 | 11.4w |
| Nov | 3,594 | **5,687** | +5,471 | 5.3w |
| Dec | 4,598 | **1,089** | +873 | 5.5w |
| Jan 2027 | 873 | **216** | +0 | 13.3w |

> **Jan 2027 ending: 216 units** · Buffer target: 216 · ✅

#### Amazon CA FBA

*Starting stock: **2,642** (0 current + + 2,642 print)*

*30-day buffer target: **51 units** (Feb avg 97 + Mar avg 57 = 51)*

| Month | Projected Sales | Ending Stock | vs Buffer | Weeks of Cover |
| :--- | ---: | ---: | ---: | ---: |
| May 2026 | 0 | **2,642** | +2,591 | 62.6w |
| Jun | 187 | **2,455** | +2,404 | 328.8w |
| Jul | 32 | **2,423** | +2,372 | 92.5w |
| Aug | 116 | **2,307** | +2,256 | 87.3w |
| Sep | 117 | **2,190** | +2,139 | 98.8w |
| Oct | 95 | **2,095** | +2,044 | 8.2w |
| Nov | 1,136 | **959** | +908 | 5.5w |
| Dec | 747 | **212** | +161 | 5.8w |
| Jan 2027 | 161 | **51** | +0 | 13.3w |

> **Jan 2027 ending: 51 units** · Buffer target: 51 · ✅

#### US Shopify

*Starting stock: **15,083** (HBG+SLI+SAV+KCM: 15,083)*

*30-day buffer target: **149 units** (Feb avg 314 + Mar avg 134 = 149)*

| Month | Projected Sales | Ending Stock | vs Buffer | Weeks of Cover |
| :--- | ---: | ---: | ---: | ---: |
| May 2026 | 0 | **15,083** | +14,934 | 59.1w |
| Jun | 1,130 | **13,953** | +13,804 | 261.1w |
| Jul | 229 | **13,724** | +13,575 | 110.3w |
| Aug | 551 | **13,173** | +13,024 | 69.0w |
| Sep | 846 | **12,327** | +12,178 | 75.3w |
| Oct | 702 | **11,625** | +11,476 | 9.9w |
| Nov | 5,188 | **6,437** | +6,288 | 6.1w |
| Dec | 4,530 | **1,907** | +1,758 | 10.2w |
| Jan 2027 | 831 | **1,076** | +927 | 95.7w |

> **Jan 2027 ending: 1,076 units** · Buffer target: 149 · ✅

#### CA Shopify

*Starting stock: **2,642** (CA hub: 1,547 + 1,095 print)*

*30-day buffer target: **51 units** (Feb avg 97 + Mar avg 57 = 51)*

| Month | Projected Sales | Ending Stock | vs Buffer | Weeks of Cover |
| :--- | ---: | ---: | ---: | ---: |
| May 2026 | 0 | **2,642** | +2,591 | 62.6w |
| Jun | 187 | **2,455** | +2,404 | 328.8w |
| Jul | 32 | **2,423** | +2,372 | 92.5w |
| Aug | 116 | **2,307** | +2,256 | 87.3w |
| Sep | 117 | **2,190** | +2,139 | 98.8w |
| Oct | 95 | **2,095** | +2,044 | 8.2w |
| Nov | 1,136 | **959** | +908 | 5.5w |
| Dec | 747 | **212** | +161 | 5.8w |
| Jan 2027 | 161 | **51** | +0 | 13.3w |

> **Jan 2027 ending: 51 units** · Buffer target: 51 · ✅

#### UK

*Starting stock: **3,975** (3,368 current)*

*30-day buffer target: **67 units** (Feb avg 98 + Mar avg 105 = 67)*

| Month | Projected Sales | Ending Stock | vs Buffer | Weeks of Cover |
| :--- | ---: | ---: | ---: | ---: |
| May 2026 | 0 | **3,975** | +3,908 | 98.4w |
| Jun | 179 | **3,796** | +3,729 | — |
| Jul | 0 | **3,796** | +3,729 | 1868.2w |
| Aug | 9 | **3,787** | +3,720 | 8387.0w |
| Sep | 2 | **3,785** | +3,718 | 228.5w |
| Oct | 71 | **3,714** | +3,647 | 6.7w |
| Nov | 2,439 | **1,275** | +1,208 | 6.0w |
| Dec | 917 | **358** | +291 | 5.5w |
| Jan 2027 | 291 | **67** | +0 | 13.3w |

> **Jan 2027 ending: 67 units** · Buffer target: 67 · ✅

#### AU

*Starting stock: **6,345** (3,128 current + + 3,217 print)*

*30-day buffer target: **78 units** (Feb avg 115 + Mar avg 121 = 78)*

| Month | Projected Sales | Ending Stock | vs Buffer | Weeks of Cover |
| :--- | ---: | ---: | ---: | ---: |
| May 2026 | 0 | **6,345** | +6,267 | 77.2w |
| Jun | 364 | **5,981** | +5,903 | 163.3w |
| Jul | 157 | **5,824** | +5,746 | 50.7w |
| Aug | 509 | **5,315** | +5,237 | 62.8w |
| Sep | 375 | **4,940** | +4,862 | 49.5w |
| Oct | 428 | **4,512** | +4,434 | 8.0w |
| Nov | 2,490 | **2,022** | +1,944 | 5.8w |
| Dec | 1,505 | **517** | +439 | 5.2w |
| Jan 2027 | 439 | **78** | +0 | 13.3w |

> **Jan 2027 ending: 78 units** · Buffer target: 78 · ✅

---

## Section 3: Print Order Instructions

New print runs ship **direct from the printer to the destination warehouse**. Do not route prints through a hub — prints going to FBA should be addressed directly to FBA; prints going to AU should go to the AU warehouse.

Place orders immediately. Standard lead times (4–8 weeks production + 4–6 weeks transit) mean orders placed in May arrive July–August, giving time for FBA inbound processing before peak season.

### 🖨️ Kids Journal — 28,702 Units

*Reason: globally short by 13,873 units.*

| Destination | Units to Print | Current Stock | Total Need | Gap Filled |
| :--- | ---: | ---: | ---: | :--- |
| Amazon US FBA | **20,843** | 1,935 | 22,778 | 20,843 of 20,843 deficit |
| Amazon CA FBA | **4,830** | 0 | 5,137 | 4,830 of 5,137 deficit |
| US Shopify | **3,029** | 22,728 | 25,757 | 3,029 of 3,029 deficit |

**Total: 28,702 units.** Ship all units direct from printer. No intermediate warehousing.

### 🖨️ Know Me If You Can Cards — 6,730 Units

*Reason: globally short by 5,805 units.*

| Destination | Units to Print | Current Stock | Total Need | Gap Filled |
| :--- | ---: | ---: | ---: | :--- |
| Amazon CA FBA | **2,642** | 0 | 2,642 | 2,642 of 2,642 deficit |
| CA Shopify | **1,095** | 1,547 | 2,642 | 1,095 of 1,095 deficit |
| UK | **607** | 3,368 | 3,975 | 607 of 607 deficit |
| EU | **35** | 681 | 716 | 35 of 35 deficit |
| AU | **2,351** | 3,128 | 6,345 | 2,351 of 3,217 deficit (UK→AU blocked) |

**Total: 6,730 units.** Ship all units direct from printer. No intermediate warehousing.

### 🖨️ Top-Up Prints

Top-up prints are small, targeted orders for channels where the transfer route is blocked or hub surplus wasn't enough. These happen **in parallel** with hub→FBA transfers — they don't replace each other.

| SKU | Destination | Units | Why Transfer Wasn't Enough |
| :--- | :--- | ---: | :--- |
| Teen Journal | Amazon US FBA | **7,032** | US hub surplus only covered 1,038 · FBA still needs 7,032 |
| Teen Journal | Amazon CA FBA | **2,750** | CA hub surplus exhausted after Shopify reserve · FBA still needs 2,750 |
| Sharing Joy Conversation Cards | Amazon US FBA | **1,401** | US hub surplus only covered 0 · FBA still needs 1,401 |
| Sharing Joy Conversation Cards | Amazon CA FBA | **901** | CA hub surplus exhausted after Shopify reserve · FBA still needs 901 |
| Sharing Joy Conversation Cards | US Shopify | **2,514** | US hubs short on their own Shopify demand · need 2,514 more at hubs |
| Sharing Joy Conversation Cards | AU | **430** | Cards can't use UK→AU route · no transfer source available |
| Daily Journal (Teal) | AU | **1,797** | Transfer routes exhausted · 1,797 unit gap remains |
| Dream Affirmation Cards | Amazon CA FBA | **1,593** | CA hub surplus exhausted after Shopify reserve · FBA still needs 1,593 |

---

## Section 4: Transfer Plan

**Deadline: September 1, 2026.** FBA inbound processing takes 2–4 weeks — stock must be checked in to FBA by early October to be available for the November–December peak. Stock arriving after mid-October may miss the window.

Hub→FBA transfers only apply to SKUs **not** getting a print run. For printing SKUs, FBA is filled by the print run directly.

| SKU | Transfer | Units | Source Stock | Dest Need | Why |
| :--- | :--- | ---: | ---: | ---: | :--- |
| Kids Journal | UK → AU | **5,066** | 33,137 | 14,045 | AU needs 14,045 (demand + buffer); AU has 8,979; deficit 5,066; UK surplus 19,679 |
| Teen Journal | US Hub → Amazon US FBA | **1,038** | 14,693 | 18,354 | US hubs have 14,693; Shopify needs 13,654 (demand + buffer); surplus 1,038; US FBA gap 8,070 |
| Sharing Joy Conversation Cards | CA Hub → Amazon CA FBA | **1,961** | 4,825 | 2,863 | CA hub has 4,825; CA Shopify needs 2,863 (demand + buffer); surplus 1,961; CA FBA gap 2,863 |
| Daily Journal (Teal) | UK → AU | **1,221** | 12,161 | 9,260 | AU needs 9,260 (demand + buffer); AU has 6,241; deficit 3,019; UK surplus 1,221 |
| Daily Journal (Teal) | US Hub → Amazon US FBA | **10,185** | 34,801 | 11,235 | US hubs have 34,801; Shopify needs 14,272 (demand + buffer); surplus 20,528; US FBA gap 10,185 |
| Daily Journal (Teal) | CA Hub → Amazon CA FBA | **1,429** | 4,526 | 2,929 | CA hub has 4,526; CA Shopify needs 2,929 (demand + buffer); surplus 1,596; CA FBA gap 1,429 |
| Daily Journal (Green) | US Hub → Amazon US FBA | **6,078** | 19,209 | 7,196 | US hubs have 19,209; Shopify needs 5,668 (demand + buffer); surplus 13,540; US FBA gap 6,078 |
| Daily Journal (Green) | CA Hub → Amazon CA FBA | **954** | 4,274 | 954 | CA hub has 4,274; CA Shopify needs 954 (demand + buffer); surplus 3,320; CA FBA gap 954 |
| Adult Journal | US Hub → Amazon US FBA | **5,302** | 37,634 | 5,717 | US hubs have 37,634; Shopify needs 7,249 (demand + buffer); surplus 30,384; US FBA gap 5,302 |
| Adult Journal | CA Hub → Amazon CA FBA | **190** | 4,929 | 1,662 | CA hub has 4,929; CA Shopify needs 1,662 (demand + buffer); surplus 3,266; CA FBA gap 190 |
| Dream Affirmation Cards | UK → EU | **523** | 9,234 | 1,466 | EU needs 1,466; EU has 943; deficit 523; UK surplus 3,009 |
| Dream Affirmation Cards | US Hub → Amazon US FBA | **3,514** | 25,105 | 4,308 | US hubs have 25,105; Shopify needs 10,951 (demand + buffer); surplus 14,153; US FBA gap 3,514 |
| Dream Affirmation Cards | CA Hub → Amazon CA FBA | **440** | 2,474 | 2,033 | CA hub has 2,474; CA Shopify needs 2,033 (demand + buffer); surplus 440; CA FBA gap 2,033 |

---

## Section 5: Action Checklist

### 🖨️ Print Orders — Place Now

- [ ] **Kids Journal** — order 28,702 units
  - [ ] 20,843 units → Amazon US FBA (direct from printer)
  - [ ] 4,830 units → Amazon CA FBA (direct from printer)
  - [ ] 3,029 units → US Shopify (direct from printer)
- [ ] **Know Me If You Can Cards** — order 6,730 units
  - [ ] 2,642 units → Amazon CA FBA (direct from printer)
  - [ ] 1,095 units → CA Shopify (direct from printer)
  - [ ] 607 units → UK (direct from printer)
  - [ ] 35 units → EU (direct from printer)
  - [ ] 2,351 units → AU (direct from printer)

### 🖨️ Top-Up Prints — Place Now

- [ ] **Teen Journal** top-up prints:
  - [ ] 7,032 units → Amazon US FBA
  - [ ] 2,750 units → Amazon CA FBA
- [ ] **Sharing Joy Conversation Cards** top-up prints:
  - [ ] 1,401 units → Amazon US FBA
  - [ ] 901 units → Amazon CA FBA
  - [ ] 2,514 units → US Shopify
  - [ ] 430 units → AU
- [ ] **Daily Journal (Teal)** top-up prints:
  - [ ] 1,797 units → AU
- [ ] **Dream Affirmation Cards** top-up prints:
  - [ ] 1,593 units → Amazon CA FBA

### 📦 Transfers — Complete by September 1

**Kids Journal:**
- [ ] UK → AU: 5,066 units

**Teen Journal:**
- [ ] US Hub → Amazon US FBA: 1,038 units

**Sharing Joy Conversation Cards:**
- [ ] CA Hub → Amazon CA FBA: 1,961 units

**Daily Journal (Teal):**
- [ ] UK → AU: 1,221 units
- [ ] US Hub → Amazon US FBA: 10,185 units
- [ ] CA Hub → Amazon CA FBA: 1,429 units

**Daily Journal (Green):**
- [ ] US Hub → Amazon US FBA: 6,078 units
- [ ] CA Hub → Amazon CA FBA: 954 units

**Adult Journal:**
- [ ] US Hub → Amazon US FBA: 5,302 units
- [ ] CA Hub → Amazon CA FBA: 190 units

**Dream Affirmation Cards:**
- [ ] UK → EU: 523 units
- [ ] US Hub → Amazon US FBA: 3,514 units
- [ ] CA Hub → Amazon CA FBA: 440 units

### 📋 Verification Checkpoints

- [ ] Print order lead times confirmed — stock must arrive by August for FBA prep
- [ ] FBA inbound shipments created in Seller Central with estimated arrival dates
- [ ] UK→AU journal transfers cleared customs and confirmed in AU system
- [ ] US hub transfers: confirm Shopify buffer reserved before moving surplus to FBA
- [ ] **Re-run this plan in August** — adjust if Q3 demand tracks above or below forecast
- [ ] **Check again in November** — flag early if December is pacing above forecast

---

*Plan generated May 3, 2026 · Model: max(2024, 2025) per month per channel · Source: `.tmp/data.json`*
